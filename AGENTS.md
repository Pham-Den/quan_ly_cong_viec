...
- **Companion tool bundles**: `tools/**` and `.tools/**` (bundled standalone tools and their install caches, e.g. the docs viewer and its `node_modules`) are tools to run, not content to read — never load their files into context; launch them via their documented launchers only when the user explicitly asks.
- **PRISM framework files**: Treat `.prism/**` and framework source equivalents (`core/**`, `adapters/**`, `docs/**`, `setup.sh`, `prism.json`, `README*.md`, `scripts/release.sh`) as read-only during normal project work. If asked to modify them, STOP and confirm the user is intentionally editing PRISM itself rather than project artifacts such as `docs/sprint-v{X}/**`, `docs/inbox/**`, the live project-root `prism-config.md`, or user-owned project notes like `docs/product-backlog.md`.
...
## Traceable-Origin Gate (ORIGIN-1, always active)

Before the FIRST create/edit/delete in a turn of anything that **affects behavior/build** (source code, repo tests, `Dockerfile`, `package.json`, CI, ...) OR a **governed PRISM artifact** (product/design/arch/plan/test-plan/test-cases proposals, change packs, bug records), resolve a tracked origin. Purely informational files (`README`, `LICENSE`, `CHANGELOG`, comments, ...) are NOT gated. `status` / `validate *` / read-only inspect are not mutations.

1. **Detect state** (cheap, cache-aware): implement pass open (plan APPROVED & sprint not sealed)? implement lane active? open change pack? open bug? sprint sealed?
2. **Classify + route**:
   - **DOC** (governed artifact) → by state: none → `start [phase]` / `import` / `start change:` / `start bug:`; DRAFT → `feedback: [phase]` / `import [phase]` (new inbox material folds into the draft); APPROVED in open sprint → `start change:` / `import [phase]` (new inbox material folds into a change pack); sealed → newest open sprint, else `new sprint`. Looks like a spec but ownership unresolved → STOP + classify (if NOT a PRISM artifact, edit per clear intent).
   - **CODE** (source / repo test) → OK only if (a) an implement pass is open and the request resolves via existing Implement routing (`start implement` / `feedback: implement`; ORIGIN-1 adds no new task-slice requirement), or (b) a DRAFT/ACTIVE bug record **with declared scope** exists. Otherwise STOP. (Whether the edit stays inside the implement/bug scope is caught afterwards by `validate bug` / `CODE-5` / Scope Discipline — ORIGIN-1 only checks the lane exists.)
3. **No origin → STOP; do not edit silently.** Show ≤3 lines of state + the two doors and ask: New requirement / spec change → `start [phase]` | `start change:` | `new sprint` | `start implement: <task-slice>` · Defect (reproducible) → `start bug: <symptom>`.
4. **Ambiguous** (DOC vs CODE, new-behavior vs bug, multiple active lanes) → STOP + ask. Never guess.

ORIGIN-1 is the single choke point: phase engines enforce in-scope quality / boundary (`CODE-5`, Scope Discipline) only AFTER it grants the lane — they do not add their own code-edit blocks. Full rule: `core/orchestrator.md § Traceable-Origin Gate (ORIGIN-1)`; bug lifecycle: `core/bug-manager.md`.
...
- "note this for later / save for a future sprint / backlog this idea" → offer to save it to the Product Backlog (`docs/product-backlog.md`) per `core/phase-product.md § Product Backlog (PROD-6)`; write only on explicit / confident intent, else ask
...
- "fix it / it's broken / crashing / wrong" (behavior contrary to what should already hold) → `start bug:` (confirm it is a defect, not a new feature; capture repro / expected / actual into the bug record before editing code)
- "implement X / build / add this feature / write the code" → requirement door via existing Implement routing (`start implement` / `feedback: implement`); if no approved plan / valid implement lane → `ORIGIN-1` STOP + ask
...
Commands: `start [phase]`, `start change: [text]`, `start plan`, `approve [phase]`, `approve changes [pack-id|slug]`, `validate changes [pack-id|slug]`, `feedback: [text]`, `status`, `new sprint`, `diff [phase]`, `resume` / `continue`, `import [phase]`, `validate user story`, `validate design`, `validate architecture`, `validate plan`, `validate test`, `validate implementation --mode spec`, `validate implementation --mode quality`, `validate implementation` (aggregate: both modes, spec first), `start bug: [text]`, `validate bug [BUG-NNN|slug]`, `approve bug [BUG-NNN|slug]`

For architecture commands, `arch` and `architecture` are equivalent.

`validate *` is the only AI audit command family. There is no separate `review [phase]` command. If the user asks to review or audit a phase, normalize that request to the matching `validate *` command. Validate commands are user-invoked, read-only audits against code and phase artifacts, required before the matching `approve [phase]`. Each run writes or updates one active validate file per underlying command in `docs/sprint-v{X}/tempo/in-progress/` (aggregate forms own no file — they update the files of the commands they run). That file must satisfy `VAL-1`: include target fingerprint, structural coverage against required sections / fields (`DOC-3`), rule coverage by Rule ID including `SEM-1`, semantic integrity evidence, findings, and conclusion. The `SEM-1` pass is LLM-only: compare the current target against effective truth and relevant upstream / downstream contracts for contradictions, duplicated intent under different IDs, terminology drift, stale assumptions, semantic cross-sprint drift, and redundant / out-of-scope content. If a validate run has any blocker, missing required `DOC-3` / Rule Coverage / `SEM-1` evidence, or missing phase-specific hard-rule coverage, it MUST conclude `issues-found`; it MUST NOT report `clean`, `pass`, or approval-ready. `validate changes [pack-id|slug]` is an aggregate convenience command, not a separate audit surface: it resolves the selected DRAFT change pack, re-derives the impacted phases per `core/change-propagation.md § Impact Matrix` BEFORE reading the pack's own `impact-matrix.md` (the pack's matrix is a generation-turn claim to verify — a mismatch is a blocker finding), and runs the existing phase validate commands for cycle `pack-<slug>`. `approve [phase]` and `approve changes` refuse to begin if the required active validate file is missing, stale, missing `VAL-1` evidence, or still has blocker findings from the latest explicit validate run. If those baseline files are clean, approval then re-runs the required validate command(s) in console-only mode as a final full confirmation pass — the re-run executes the full command contract, including the `VAL-3` / `VAL-2` independence conduct (fresh-context reviewer agent by default); console-only is not a lighter audit. For `approve implement`, BOTH `--mode spec` AND `--mode quality` must satisfy these conditions — running only one is not sufficient. Document validates (`validate user story` / `design` / `architecture` / `plan` / `test`) additionally satisfy `VAL-3` (canonical conduct: `core/phase-quality-standards.md § General Standards`): spawn a fresh-context reviewer agent by default (fall back to same-context conduct only when the environment cannot open one; the reviewer prompt carries only file paths and identifiers — never the generating turn's claims), re-read the target / template / upstream approved artifacts from files (never from conversation memory), re-derive expectations before reading generation-turn claims, grade each required section / rule ID by searching for its fail signal, and record the expectation sources in the active validate file. `validate implementation --mode spec` additionally satisfies `VAL-2` (canonical protocol: `core/phase-implement.md § Independent verification protocol`): apply the four protocol pillars (fresh eyes — a fresh-context reviewer agent by default; re-derived expectations; per-AC refutation; re-run, don't trust) and run `python .prism/core/tools/validate_implementation.py`, recording its deterministic findings in the active validate file. `validate implementation` with no `--mode` is an aggregate convenience form (same pattern as `validate changes`): it runs `--mode spec` then `--mode quality` in one pass, updating both active validate files; it is not a separate audit surface and owns no validate file of its own.

Same-sprint corrections on APPROVED artifacts use `start change:`. `validate changes [pack-id|slug]` audits the selected DRAFT change pack by running the existing phase validate command(s) for every impacted phase up to the current downstream phase; it writes normal files such as `validate-design-pack-<slug>.md`, not `validate-changes-<slug>.md`. The validation target is the selected pack's proposed truth: base artifact + all APPROVED deltas + this selected DRAFT pack's delta. Do not validate only the frozen base artifact, and do not include DRAFT deltas from unrelated packs. `approve changes` closes the selected DRAFT change pack only after those pack-cycle validate files are clean and approval-time re-runs find no new gaps. If more than one DRAFT pack exists, ask the user to choose by sprint, pack id, or slug. Both `validate changes` and `approve changes` also run the same-sprint collision check (`core/change-manager.md § Same-Sprint Collision Check`) and surface any anchor changed by 2+ sources this sprint, so a clash is reconciled or confirmed early — not discovered only when the sprint seal hard-blocks it. They likewise run the deterministic pack-structure check (`core/change-manager.md § Pack Structure Check` — `python .prism/core/tools/seal_sprint.py --sprint v{X} --check-pack-structure --pack <id>`), which verifies `change-request.md` + `impact-matrix.md` exist and parse, the matrix's delta rows agree two-way with the delta files actually present, and every delta passes `validate_proposal.py`; tool findings are validate blockers.
...
| test | `approve` | product + design + architecture APPROVED, all previous sprints sealed, Test rules `TEST-1`, `TEST-2`, `TEST-3`, `TEST-3b`, `TEST-3c`, `TEST-4`, `TEST-5`, `TEST-6`, `TEST-7`, and `TEST-8` satisfied (coverage traceability, execution-ready + implementation-consumable handoff, rule/branch inventory, trace + Technique title-prefix discipline, architecture-derived obligations, functional/SIT coverage, data requirements, automation intent, external QA handoff, generated TSV exports), active `validate test` file is clean + approval-time re-run finds no new gaps + `validate_proposal.py --file docs/sprint-v{X}/testing/proposals/test-cases-v{X}.md` returns 0 blockers |
...
**Reflect sprints** (`kind: reflect`, brownfield onboarding) are the exception to the build/spec gates above: the engine waives the build/spec checks for an as-built bundle, while the structural seal validators (`validate_proposal.py` + post-merge `validate_living_truth.py`) stay on. Follow the engine — the exact carve-out is defined in `core/sprint-manager.md § Reflect Sprint` + each `phase-*.md` Gate (loaded via Phase Engine Loading), not restated here.

Brownfield onboarding is **implicit** (no command, no hand-edit): when a requirement touches an area the Codebase Map (`docs/codebase-map.md`) shows as exists-but-un-governed, or the user asks to bring existing code under PRISM, the engine opens a reflect sprint to onboard that area FIRST — loading the bundled `discover-code` skill (`.prism/skills/discover-code/SKILL.md`, the only component that reads code) per `core/sprint-manager.md § Codebase Map` (which also governs the onboard-before-build gate, mid-sprint add-vs-missing-dependency handling, and the genesis-vs-incremental ask). If that skill isn't installed, say so. **Greenfield (no Codebase Map) never enters this path** and never loads the skill.

## Import / Resume Rules

- `import [phase]`: Accepts complete, partial, or in-progress documents. Audit imported material against the phase standard, classify what is accepted, normalized, needs improvement, missing, unclear, conflicting, or redundant, and ask only the targeted follow-up questions that remain for missing, unclear, below-quality, conflicting, or keep / drop items. If any items remain unresolved after the clarification batch, PRISM appends an `## Open Issues` section to the DRAFT. On a phase already APPROVED in an open sprint, the same audit runs and the synthesis folds into a change pack instead of proposals (`core/import-validator.md § Import Into A Change Pack`).
...
If the user tries to modify an APPROVED document, route to `start change:` on the sprint that owns it. If that sprint is sealed, block with: `⚠ [phase] cannot be changed in this sealed sprint. Make the change in the newest open sprint; if none is open, new sprint. To inspect differences first: diff [phase]`.

**Cross-sprint routing when several sprints are open** (every routing command resolves its target this way): new work with no named target defaults to the **newest open sprint**; a **sealed** sprint is read-only (→ newest open sprint or `new sprint`); an explicitly named or in-progress **unsealed** sprint — older or newer — is a valid target when the command's preconditions hold there, so **being *older* is not a refusal**. If a change in an older sprint touches an anchor ID a newer open sprint also carries, warn the user to **re-review that newer sprint** — the overlap is advisory and, left unreconciled, can be overwritten when the newer sprint seals. If more than one open sprint is a plausible target, ask. Never auto-edit a newer sprint.
...
- `resume` / `continue`: detect the latest DRAFT document across the open sprints (continuation per cross-sprint routing — not only the current sprint; ask if more than one is plausible); if planning and testing are tied across independent lanes, show the standard lane-choice block before continuing. If the selected planning DRAFT contains `dependencies_pending`, re-check those prerequisite phases before approval.
...
- **Feature Linkage Is Mandatory**: Every new or materially changed in-scope meaningful backend/runtime, frontend/UI, or AI/ML code surface must satisfy the canonical `CODE-1 Traceability Contract` in `core/phase-implement.md §6` (loaded on implement activation). Include explicit pack / `Tracking IDs / ticket / task / board IDs` only when provided — never invent them. Preserve or update these markers on `feedback:`. Do not spray boilerplate markers across trivial helpers (`CODE-2`).
...
- **Consistent Patterns**: Read existing codebase first. Follow existing patterns. `CODE-1` overrides local no-comment legacy style; missing old markers are not a pattern to copy. Don't create new patterns without user approval
...
| `start bug:`, `feedback bug:`, `validate bug`, `approve bug` (and any code mutation `ORIGIN-1` routes to the bug lane) | `.prism/core/bug-manager.md`, `.prism/core/templates/bug-report-template.md` |
