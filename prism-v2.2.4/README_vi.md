...
flowchart TD
    ...
    Q -->|"🔥 Bug production gấp"| H["start bug: HOTFIX … (severity urgent) → fix trên hotfix/… → validate/approve bug"]
...
| 🔥 Hotfix production | Bug đang chạy trên prod | `start bug: [HOTFIX] …` (urgent) → fix trên `hotfix/…` → `approve bug` | Luồng bug | [↓](#scenario-hotfix) |
...
File zip sẽ bung ra thành thư mục `.prism/` ở root project, kèm `README.md`, `README_vi.md`, `diagram.xml` ở root để xem hướng dẫn ngay, và có thêm `qa-tools/` và `tools/docs-viewer/` nếu release đi kèm các companion tool standalone.
...
- `tools/docs-viewer/` là docs viewer standalone tuỳ chọn cho `./docs`; PRISM core không load thư mục này
...
### Docs viewer tuỳ chọn

Nếu release có `tools/docs-viewer/`, chạy launcher từ project root để browse `./docs` local:
...
Cần Node.js 20 trở lên. Launcher cài bundled viewer vào `.tools/docs-viewer-install/`, nên `package.json`, `package-lock.json`, và `node_modules/` của npm không nằm ở project root. Viewer này là companion tool standalone, không phải runtime của PRISM trong `.prism/`.
...
**Lưu ý:** Import dùng được ở mọi giai đoạn. Khi phase còn là DRAFT, tài liệu import được merge vào draft đó. Nếu phase **đã approve** trong sprint đang mở, PRISM tự ghi nhận tài liệu import thành một change pack giữa sprint — impact lên các phase sau được đánh giá tự động, và bạn có thể bỏ pack đó bất kỳ lúc nào trước khi approve. Import **không bao giờ tự approve** — kết quả luôn là DRAFT để bạn review. Xem thêm quy ước đặt tên file ở mục [Import Tài Liệu Có Sẵn](#import-tai-lieu-co-san).
...
**Khi nào:** Dự án đã có codebase đang chạy và bạn muốn đưa nó vào quy trình PRISM. Chọn hướng theo mục tiêu.

**A. Đưa code hiện có vào governance (reflect onboarding).** Dùng khi bạn muốn chính code hiện tại được ghi nhận thành Living Truth để tiếp tục phát triển trên nền đó.
1. Chạy skill **`discover-code`** (đóng kèm trong `.prism/skills/` — thành phần DUY NHẤT đọc code). Nó suy-ngược vùng được chọn ra `docs/inbox/` + `docs/codebase-map.md`, mọi claim đều dẫn `path:line`, không bịa. Bỏ tài liệu cũ (PRD/SAD/wiki cũ) vào **`docs/legacy/`** để nó đối soát với code — đây là vị trí mặc định, **không bắt buộc**; cứ báo AI nếu tài liệu ở chỗ khác (AI hỏi nếu chưa rõ).
2. AI tự mở **reflect sprint** cho vùng đó (`kind: reflect` — đặt tự động, không hand-edit) rồi `import` inbox thành proposals — cấp ID và mang theo provenance `reflected` — tắt cổng build/spec nhưng **mọi validator cấu trúc vẫn bật**.
3. `validate` + `approve` từng phase, rồi `approve implement` để **seal** spec as-built vào Living Truth. Repo lớn thì onboard từng vùng mỗi reflect sprint; repo nhỏ thì ôm cả repo một lần.

> **Phần intent code không cho biết thì hoãn lại, không bịa, không chặn.** Code cho thấy *cái gì* chạy, không cho biết *vì sao* — nên mục tiêu, KPI, lý do, threat model được ghi thành open question / risk có theo dõi (trong các mục deferral của PRD / architecture), không đoán bừa, và onboarding **không** bị chặn vì chúng. Chúng nổi lại để product owner điền khi bạn build/đổi vùng đó lần sau. (Greenfield và việc thêm tính năng mới vẫn hỏi bạn ngay từ đầu.)

**B. Định nghĩa spec đích rồi build tiếp (spec-first).** Dùng khi bạn chủ yếu muốn thêm/redesign tính năng chứ không phải ghi lại hiện trạng.
1. `import arch` để nắm kiến trúc **hiện trạng** (bỏ SAD/ADR/sơ đồ/API sketch hiện có vào `docs/inbox/`).
2. Chạy Product / Design / Architecture để định nghĩa **spec đích** (trạng thái mong muốn).
3. `start implement` để PRISM viết code mới bám spec đã approve.

**Lưu ý:** PRISM vẫn **spec-first** — Living Truth chỉ đổi tại sprint seal, và Safety Guard vẫn bảo vệ code hiện có. Reflect onboarding (hướng A) đưa hiện trạng as-built vào governance qua đúng pipeline seal đó: nó **mô tả** code (không sửa), và chỉ `discover-code` được đọc code.

<a id="scenario-hotfix"></a>
### 🔥 Hotfix — Bug Production Gấp

**Khi nào:** Bug đang chạy trên production, cần fix gấp. **Chỉ sửa bug, không thêm tính năng.** Hotfix là biến thể `severity: urgent` của luồng bug.

**Các bước:**
1. **Mở bug trước:** gõ `start bug: [HOTFIX] <triệu chứng>` và đánh dấu urgent. PRISM tạo bản ghi `BUG-NNN` (triệu chứng, repro, expected vs actual) **trước khi** sửa code, để fix luôn được track.
2. **Fix code:** fix có thể đi trên branch `hotfix/{mô-tả}` từ `main` và bỏ qua chu kỳ sprint thông thường cho nhanh, nhưng vẫn đi qua bản ghi bug → `validate bug` (repro tái hiện được + regression test pass) → `approve bug`. Kèm 1 regression test gắn `VERIFIES: BUG-NNN`. Safety Guard + Git hygiene vẫn áp dụng đầy đủ.
3. **Phản ánh tác động tài liệu (nếu có):** nếu fix chạm requirement / design / architecture, doc-impact của bug định tuyến sang `start change: [HOTFIX] <tóm tắt>` trong sprint (đang mở) bị ảnh hưởng — hoặc `new sprint` → change trước nếu sprint mới nhất đã seal (không thể mở change pack trong sprint đã seal). Change quay lại Living Truth khi sprint đó seal.
...
│   ├── skills/            skill đi kèm mà framework tham chiếu
│   │   └── discover-code/ đọc code có sẵn để suy-ngược tài liệu (brownfield onboarding)
...
├── tools/                 companion tool tuỳ chọn, không được PRISM core load
│   └── docs-viewer/       viewer local cho ./docs của project
...
│   ├── product-backlog.md ý tưởng sản phẩm tương lai chưa thuộc sprint nào (## Active / ## Archive; user sở hữu, không gate)
...
Nếu project của bạn đã có sẵn thư mục `docs/`, PRISM chỉ thêm `docs/inbox/` và `docs/sprint-v1/`. Cây Living Truth (per-phase nested) dưới `docs/product/`, `docs/design/`, `docs/architecture/`, `docs/testing/` (gồm 15 file LT root + per-epic files) được thêm vào lần đầu `seal_sprint.py` chạy. `docs/product-backlog.md` được tạo lazily lần đầu bạn ghi một ý tưởng sản phẩm tương lai — user sở hữu, không bao giờ gate phase. PRISM không đè phần tài liệu khác của project.
...
### 5b. Khi sửa Bug (defect trong code đã chấp nhận)
| Lệnh | Khi nào dùng |
|---|---|
| `start bug: [triệu chứng]` | Khi code đã được chấp nhận/seal nhưng không làm đúng spec (spec vẫn đúng) — một defect tái hiện được. Mở bản ghi `BUG-NNN` độc lập sprint; là origin hợp lệ cho việc sửa code. Hotfix production là `start bug:` với severity urgent. |
| `validate bug [bug-id-hoặc-slug]` | Audit một bug DRAFT/ACTIVE trước `approve bug`: repro tái hiện (hoặc giải trình substantive), regression test (`VERIFIES: BUG-NNN`) pass, fix trong scope đã khai, doc-impact đã đánh giá. |
| `approve bug [bug-id-hoặc-slug]` | Đóng bug (`RESOLVED`) khi validate sạch, regression test pass, và doc-impact (nếu có) đã xử lý. Không seal gì cả. |
...
Gõ `start bug: [HOTFIX] …` (severity urgent) — PRISM ghi nhận bug trước, sau đó bạn fix trên branch `hotfix/…` rồi clear `validate bug` / `approve bug`; tác động tài liệu (nếu có) định tuyến qua `start change: [HOTFIX] …`. Xem [Hotfix](#scenario-hotfix).
