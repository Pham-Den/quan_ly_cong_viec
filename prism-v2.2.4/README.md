...
flowchart TD
    ...
    Q -->|"🔥 Urgent production bug"| H["start bug: HOTFIX … (severity urgent) → fix on hotfix/… → validate/approve bug"]
...
| 🔥 Production hotfix | Bug live in production | `start bug: [HOTFIX] …` (urgent) → fix on `hotfix/…` → `approve bug` | Bug lane | [↓](#scenario-hotfix) |
...
The zip expands to `.prism/` at the project root, plus root-level `README.md`, `README_vi.md`, `diagram.xml`, and `qa-tools/` and `tools/docs-viewer/` when the release includes optional standalone companion tools. Open the root README first if you need setup commands before running the installer.
...
- `tools/docs-viewer/` as an optional standalone docs viewer for project `./docs`; PRISM core does not load it
...
If the release includes `tools/docs-viewer/`, launch it from the project root to browse `./docs` locally:
...
Node.js 20 or newer is required. The launcher installs the bundled viewer under `.tools/docs-viewer-install/`, so npm's `package.json`, `package-lock.json`, and `node_modules/` stay out of the project root. The viewer is a standalone companion tool, not part of the PRISM runtime under `.prism/`.
...
**Note:** Import works at any stage. While the phase is still a draft, imported material merges into that draft. If the phase is **already approved** in the open sprint, PRISM records the imported material as a mid-sprint change pack instead — the impact on later phases is assessed automatically, and you can discard the pack any time before approving it. Import **never auto-approves** — the result is always a DRAFT for you to review. See file-naming details in [Import File Naming Reference](#import-existing-documents).
...
**When:** The project already has a running codebase and you want to bring it into the PRISM workflow. Pick the path that matches your goal.

**A. Bring the existing code under governance (reflect onboarding).** Use this when you want the current code itself captured as Living Truth so you can keep developing on top of it.
1. Run the **`discover-code`** skill (bundled under `.prism/skills/` — the only component that reads code). It reverse-documents the chosen area into `docs/inbox/` plus a `docs/codebase-map.md`, every claim cited to `path:line`, and never fabricates. Drop any existing docs (old PRD / SAD / wiki) into **`docs/legacy/`** so it can reconcile them against the code — that is the default spot, **not required**; just tell the AI if your docs live elsewhere (it asks if unsure).
2. The AI opens a **reflect sprint** for that area (`kind: reflect` — set automatically, no hand-edit) and `import`s the inbox into proposals — assigning IDs and carrying the `reflected` provenance — with build/spec gates relaxed but **all structural validators still on**.
3. `validate` + `approve` each phase, then `approve implement` to **seal** the as-built spec into Living Truth. Onboard one area per reflect sprint for a large codebase, or the whole repo at once if it is small.

> **Intent the code can't reveal is deferred, never invented or blocked.** Code shows *what* runs, not *why* — so goals, KPIs, rationale, or a threat model are recorded as tracked open questions / risks (in the PRD / architecture deferral sections), never guessed, and onboarding does **not** block on them. They resurface for a product owner to fill when you next build on or change that area. (Greenfield and new-feature work still ask you for these up front.)

**B. Define a target spec and build forward (spec-first).** Use this when you mainly want to add or redesign features rather than document the current state.
1. `import arch` to capture the **current** architecture (put existing SAD/ADR/diagrams/API sketches into `docs/inbox/`).
2. Run Product / Design / Architecture to define the **target spec** (desired state).
3. `start implement` so PRISM writes new code aligned to the approved spec.

**Note:** PRISM stays **spec-first** — Living Truth only ever changes at sprint seal, and Safety Guard still protects your existing code. Reflect onboarding (path A) brings as-built reality under governance through that same seal pipeline: it **documents** your code (it does not edit it), and only `discover-code` ever reads it.

<a id="scenario-hotfix"></a>
### 🔥 Hotfix — Urgent Production Bug

**When:** A production bug needs an urgent fix. **Bug-fix only, never new features.** A hotfix is the `severity: urgent` variant of the bug lane.

**Steps:**
1. **Open the bug first:** run `start bug: [HOTFIX] <symptom>` and mark it urgent. PRISM creates a `BUG-NNN` record (symptom, reproduction, expected vs actual) **before** any code change, so the fix stays tracked.
2. **Fix the code:** the fix may go on a `hotfix/{desc}` branch from `main` and skip the normal sprint-requirement cycle for speed, but it still flows through the bug record → `validate bug` (reproduction shown + regression test passes) → `approve bug`. Ship a regression test tagged `VERIFIES: BUG-NNN`. Safety Guard + Git hygiene stay fully active.
3. **Reflect any doc impact:** if the fix changes requirement / design / architecture, the bug's doc-impact routes to `start change: [HOTFIX] <summary>` in the affected (open) sprint — or `new sprint` → change first if the latest sprint is sealed (a change pack cannot open in a sealed sprint). The change re-enters Living Truth at that sprint's seal.
...
│   ├── skills/            bundled skills the framework references
│   │   └── discover-code/ reads existing code to reverse-document it (brownfield onboarding)
...
├── tools/                 optional companion tools not loaded by PRISM core
│   └── docs-viewer/       local viewer for project ./docs
...
│   ├── product-backlog.md unscheduled future product ideas (## Active / ## Archive; user-owned, non-gating)
...
If your project already has a `docs/` folder, PRISM only adds `docs/inbox/` and `docs/sprint-v1/`. The Living Truth tree under `docs/product/`, `docs/design/`, `docs/architecture/`, and `docs/testing/` is added the first time `seal_sprint.py` runs. `docs/product-backlog.md` is created lazily the first time you capture a future product idea — it is user-owned and never gates a phase. PRISM does not replace the rest of your project documentation.
...
### 5b. Fixing Bugs (defects in accepted code)
| Command | When to use |
|---|---|
| `start bug: [symptom]` | When code that was already accepted/sealed does not do what the (correct) spec says — a reproducible defect. Opens a sprint-independent `BUG-NNN` record; the authorized origin for the code fix. A production hotfix is `start bug:` with severity urgent. |
| `validate bug [bug-id-or-slug]` | Audit a DRAFT/ACTIVE bug before `approve bug`: reproduction shown (or substantively justified), regression test (`VERIFIES: BUG-NNN`) passes, fix in declared scope, doc-impact assessed. |
| `approve bug [bug-id-or-slug]` | Close a bug (`RESOLVED`) once validate is clean, the regression test passes, and any doc-impact is resolved. Does not seal anything. |
...
Open `start bug: [HOTFIX] …` (severity urgent) — PRISM records the bug first, then you fix on a `hotfix/…` branch and clear `validate bug` / `approve bug`; any doc impact routes through `start change: [HOTFIX] …`. See [Hotfix](#scenario-hotfix).
