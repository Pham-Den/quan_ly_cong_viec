...
- [x] 23.1 Keep the direction dev-focused: graph, side panel, search, and flow remain the main working surface.
- [x] 23.2 Keep node/dependency topology global and import runtime/config as environment bindings.
- [x] 23.3 Add settings only for UI behavior that helps daily graph/debug use.
- [x] 23.4 Add JSON/YAML import with preview before apply.
- [x] 23.5 Add export of the current System Manager topology format.
- [x] 23.6 Keep scanner, health checks, incidents, SSH, logs, Docker actions, and destructive import delete/sync out of phase 5.

## 24. Phase 5 Backend Import/Export API

- [x] 24.1 Add authenticated topology export endpoint returning environments, hosts, global nodes, environment node bindings, global dependencies, and environment dependency bindings.
- [x] 24.2 Add authenticated import preview endpoint that validates the payload and reports create/update counts without writing data.
- [x] 24.3 Add authenticated import apply endpoint that upserts environments, hosts, global nodes, node bindings, global dependencies, and dependency bindings.
- [x] 24.4 Keep import apply non-destructive: no delete/sync behavior in phase 5.
- [x] 24.5 Add backend tests for export, preview, apply, and global topology/environment binding behavior after import.

## 25. Phase 5 Frontend Settings And Import/Export UI

- [x] 25.1 Add persistent System Manager settings to local state.
- [x] 25.2 Add a settings/import/export surface in the System Manager feature.
- [x] 25.3 Let the user configure default graph view and panel behavior for local daily use.
- [x] 25.4 Let the user export the topology as JSON.
- [x] 25.5 Let the user select JSON/YAML, preview import impact, and apply only after preview.
- [x] 25.6 Refresh environments/topology after import apply so graph/search/side panel/flow use the imported data.

## 26. Phase 5 Verification And Review

- [x] 26.1 Run backend Prisma validation/build/test.
- [x] 26.2 Run frontend typecheck/build.
