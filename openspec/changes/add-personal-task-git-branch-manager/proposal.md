...
- Add app-assisted branch creation at any time with checkout source, intended merge target, branch lineage, and inherited task tracking.
- Add branch lifecycle actions for tracking task progress from feature branch to release branch to `main`.
...
- `branch-lifecycle`: Git branch records, app-assisted branch creation, checkout source tracking, intended/actual merge target tracking, branch lineage, task-branch inheritance, task-branch linking, feature-to-release-to-main lifecycle, and merge-driven task status rules.
...
- Adds branch source/target fields so each task can show where its branch was checked out from and where it has been merged.
- Adds branch lineage rules so a derived branch can carry a task forward and complete it when the derived branch reaches `main`.
