...
def build_payload(row: dict, cfg: dict, folder_id: int) -> tuple[dict, list[dict], str | None]:
    ...
    # Sync Component to QMetry testComponent field.
    component_name = row_value(row, mapping.get("component", "Component"))
    if component_name:
        payload["testComponent"] = component_name
        updates["testComponent"] = component_name
        ...
        try:
            ...
            if args.dry_run:
                ...
                report.append({
                    ...
                    "testComponent": payload.get("testComponent"),
                    "stepCount": len(steps),
                })
                continue
