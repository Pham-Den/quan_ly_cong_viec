...
def list_attachments(site_root: str, wiki_base: str, page_id: str, headers: dict[str, str]) -> dict[str, dict[str, str]]:
    ...
    # Use api.atlassian.com/ex/confluence/{cloudId} proxy — the direct /wiki/download/ path
    # requires OAuth session; the proxy accepts Basic auth (API token).
    cloud_id = ""
    for item in results:
        ari = item.get("ari", "")
        if ari:
            ...
            break
    proxy_base = f"https://api.atlassian.com/ex/confluence/{cloud_id}" if cloud_id else site_root
    ...
    for item in results:
        ...
        if title and download_rel:
            ...
            attachments[title] = {
                ...
                "download_url": f"{proxy_base}/wiki{rel_no_query}",
            }
    return attachments
