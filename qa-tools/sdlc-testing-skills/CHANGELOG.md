...
**Type:** Patch — confluence sync expand macro + Basic Flow format fixes

### Summary of changes

| # | Changed file | Type | Short description |
|---|---|---|---|
| 1 | `tools/confluence_sync.py` | **Fix** | Step 1.5 (new): pre-expand cleanup dùng innermost-first regex để loại bỏ jira/status/view-file macros trước khi step 2 expand processing; fix US template tables bên trong expand macros bị truncate do non-greedy `.*?` dừng tại inner macro closing tag |
| 2 | `tools/confluence_sync.py` | **Fix** | `_template_table_to_sections()`: đổi từ prose format (`**Label:**\ncontent`) sang 2-column markdown table format (`\| Label \| Content \|`), nhất quán với US 9-12 tables |

### Lý do thay đổi

**Vấn đề 1 (expand macro truncation):** US 1-8 template tables nằm trong `<ac:structured-macro ac:name="expand">`. Step 2 dùng non-greedy `.*?` → dừng tại `</ac:structured-macro>` đầu tiên (là inner jira/view-file macro, không phải outer expand). Chỉ extract được partial body → Precondition/Basic Flow/AC rows bị để raw HTML → render thành `**Label**` prose bởi step 11+15. Fix: step 1.5 dùng innermost-first regex (tương tự step 9 tables) để remove all non-expand macros trước khi step 2 chạy.

**Vấn đề 2 (template table prose format):** Sau khi inner "Logic xử lý" table được convert sang markdown `|` rows và embed vào US table's TD, `_is_template_table()` detect `^\s*|` → gọi `_template_table_to_sections()` → prose format thay vì table. Fix: đổi `_template_table_to_sections()` dùng `_cell_html_to_md()` và render thành `| label | value |` table format.
...
**Type:** Patch — confluence sync bug fixes (2 iterations)

### Summary of changes

| # | Changed file | Type | Short description |
|---|---|---|---|
| 1 | `tools/html_to_md.py` | **Fix** | Thêm `_pre_process_macros()`: regex-flatten `ac:structured-macro` trước khi BeautifulSoup parse; tránh RecursionError/stack overflow trên trang 300KB+ |
| 2 | `tools/confluence_sync_space.py` | **Fix** | Dùng `confluence_storage_to_markdown()` từ `confluence_sync.py` làm primary converter (pure regex, không đệ quy); sinh Markdown table đúng format, tải ảnh inline; fix `relative_to(ROOT)` ValueError khi output nằm ngoài ROOT |
| 3 | `tools/confluence_sync.py` | **Fix** | Step 13 lists: thay `_strip_tags` flattening bằng depth-aware iterative converter; nested `<ul>/<li>` giờ output `  - item` đúng indent |
| 4 | `tools/confluence_sync.py` | **Fix** | Step 15 paragraphs: thêm `\n` trước `<p>` để section headers (Precondition, Basic Flow, Acceptance Criteria, v.v.) có blank line tách biệt với content |
| 5 | `tools/confluence_sync_space.py` | **Fix** | `images_dir` thiếu `/ page_id`: ảnh tải về `images/filename` nhưng ref trong markdown là `images/{page_id}/filename` → thêm `/ page_id` để khớp |

### Lý do thay đổi

**Vấn đề 1–2 (session trước):** `confluence_sync_space.py` cũ dùng BeautifulSoup + markdownify xóa sạch nội dung expand macro → file 3KB. Thay bằng `confluence_storage_to_markdown()` từ `confluence_sync.py`.

**Vấn đề 3 (lists):** Step 13 gọi `_strip_tags` trên content của mỗi `<li>` → xóa tất cả `<ul>/<li>` nội bộ → list phẳng không indent. Fix: xử lý iterative từ innermost → outermost, đếm số `<ul>` mở trước match để tính depth.

**Vấn đề 4 (paragraphs):** Step 15 chỉ add `\n` sau `<p>` → section headers dính vào content liền dưới. Fix: thêm `\n` trước.

**Vấn đề 5 (images path):** `images_dir = page_folder / IMAGE_DIR_NAME` → files ở `images/file.png`. Ref trong markdown: `images/{page_id}/file.png`. Fix: thêm `/ page_id` vào `images_dir`.
...
**Type:** Minor — TC design philosophy overhaul (breaking change to coverage model)

### Summary of changes

| # | Changed file | Type | Short description |
|---|---|---|---|
| 1 | `skills/qa-core/05-gen-tc-functional/gen-tc-functional.md` | **Update** | Kỹ thuật thay thế đếm số: meta-principle, bỏ HLTC ratio, thêm bảng Implicit→Technique, viết lại bảng 12 group với Coverage guidance (How/Done when), ET→SFDPOT, checklist bỏ ≥2 ET/≥2 Impact |
| 2 | `references/skill-details/05-gen-tc-functional.md` | **Update** | Sync với FILE 1: Rule Inventory (ALL distinct triples), bảng group với Coverage guidance, SFDPOT ET, HLTC ratio bỏ, checklist Impact fix |
| 3 | `skills/qa-core/07-review-tc/review-tc.md` | **Update** | Coverage score từ ≥8/12 → ALL + stopping criterion; 8 SIT groups thêm stopping criterion column; gate threshold ≥90%/≥70% → ALL; Low-risk skip rule → documented risk acceptance; ET gate → SFDPOT |
| 4 | `references/skill-details/07-review-tc.md` | **Update** | Sync coverage score + gate thresholds với FILE 3 |
| 5 | `skills/qa-core/06-gen-tc-sit/gen-tc-sit.md` | **Update** | Viết lại bảng 8 SIT groups với Coverage guidance (How/Done when); ET→SFDPOT tập trung O/T/F dimensions |
| 6 | `references/skill-details/06-gen-tc-sit.md` | **Update** | Sync bảng 8 SIT groups với FILE 5 |
| 7 | `evaluation/skill-quality-rubric.md` | **Update** | ET criterion: ≥2 rows → SFDPOT; SIT rubric: ≥1 TC/flow → complete E2E TC; Part B advisory: thêm note TC count ≠ quality metric |
| 8 | `references/tc-step7-checklist.md` | **Update** | C08: ≥2 ET rows → SFDPOT-based (all 6 dimensions; 1 row/risk area; no count target) |

### Lý do thay đổi (design rationale)

**Vấn đề cũ:** Mọi nhóm TC đều có ngưỡng số lượng cố định (≥1, ≥2, ≥8/12) dẫn đến 2 xu hướng sai:
1. Viết đủ số rồi dừng → bỏ sót scenarios thực sự cần cover
2. Ngược lại, ép giảm số lượng (ví dụ "20 TC tốt hơn 60 TC") → cũng là đặt ceiling sai

**Quan điểm đúng của chuyên gia kiểm thử:** TC count là kết quả của việc áp dụng kỹ thuật, không phải mục tiêu. EP → 1 TC/equivalence class; BVA → boundary points; ST → 100% transition coverage; DT → 1 TC/combination. Một nhánh đơn giản cần 3 TC; một nhánh có state machine + multi-role auth cần 15 TC. Cả hai đều đúng.

**Cơ chế mới:** Mỗi nhóm có **How** (phương pháp liệt kê — enumerate all) và **Done when** (stopping criterion — per technique) thay thế hoàn toàn cho Minimum count.

### Breaking changes

- `Coverage score: Full` từ `≥8/12 applicable groups` → `ALL applicable groups covered; stopping criterion met per covered group`. File đang dùng ngưỡng cũ cần review lại coverage score.
- Gate threshold `≥90%/≥70%` (review-tc) → `ALL applicable Rule IDs`. Review hiện tại coi là Approved khi 90% AC happy path có TC — nay phải 100%.
- `Risk Low` skip rule (review-tc) bỏ điều kiện `≥20 supplemental TCs` — nay cần documented risk acceptance + QA Lead sign-off.
- G3 (DataDriven Consolidation) không còn là nhóm tạo TC — là bước consolidation sau G1+G2. Mark `Applied ✓` trong coverage matrix thay vì `N/A`.
...
| # | Changed file / folder | Type | Short description |
|---|---|---|---|
| 1 | `tools/confluence_sync.py` | **Fix** | Sửa image sync: dùng `api.atlassian.com/ex/confluence/{cloudId}` proxy thay vì `/wiki/download/` (cần OAuth); URL-encode filename tránh step-17 strip angle brackets; cache attachment map per page |
| 2 | `tools/confluence_fetch_with_ocr.py` | **Fix** | Sửa `list_attachments()`: thay `urljoin(site_root, ...)` → proxy URL với cloudId (tránh 404/401 khi download ảnh) |

### Chi tiết fix

**Root cause:** Confluence Cloud không cho phép download attachment qua `/wiki/download/attachments/` với Basic auth (trả về 401 OAuth). Phải dùng `https://api.atlassian.com/ex/confluence/{cloudId}/wiki{rel_path}` — cloudId lấy từ field `ari` trong response của attachment API.

**Các loại ảnh được hỗ trợ:** PNG, JPG, GIF, WebP, SVG, BMP, TIFF (bất kỳ MIME `image/*`). Non-image (PDF, DOCX) bị bỏ qua.
...
**Type:** Minor — backward-compatible additions + housekeeping

### Summary of changes

| # | Changed file / folder | Type | Short description |
|---|---|---|---|
| 1 | `skills/qa-core/09b-log-defect/log-defect.md` | **New** | Skill mới: log confirmed defect lên Jira với đầy đủ trường QC chuẩn |
| 2 | `tools/jira_create_bug.py` | **New** | Script tạo Jira Defect: sprint auto-detect từ US issue, auto link Defect→US, ADF description |
| 3 | `SKILL.md` | Update | Thêm routing entry cho `09b-log-defect` |
| 4 | `README.md` | Update | Thêm Scenario 5 (skill 09b), bảng tham số jira_create_bug.py (18 params), governance table, 4 troubleshooting entries; version bump v2.1.0 → v2.2.0 |
| 5 | `template/` | Cleanup | Xóa 52 file `.json` Confluence sidecar (không cần thiết cho AI skills) |
| 6 | `template/13 - QC Knowledge Sharing/` | Delete | Xóa folder (12 files) — nội dung project-specific, không thuộc template chuẩn |
| 7 | `tools/.env` | **Delete** | Xóa file credentials (security risk — không được commit) |
| 8 | `tools/legacy/` | Delete | Xóa 10 scripts cũ không còn sử dụng |
| 9 | `tools/__pycache__/`, `assets/.../Libs/__pycache__/` | Delete | Xóa Python bytecode cache |
| 10 | `tools/confluence_sync_space.py.bak`, `.gitignore.bak` | Delete | Xóa backup files |
| 11 | `assets/.../Libs/GoogleSheet copy.py` | Delete | Xóa file duplicate |
| 12 | `references/skill-details/appium-mcp.code-workspace` | Delete | Xóa file project-specific không thuộc skill suite |
| 13 | `CHANGELOG-v2.1.0.md` → `CHANGELOG.md` | Rename | Đổi tên về tên chuẩn |

### Skill 09b — Log Defect

Skill mới `09b-log-defect` bổ sung vào luồng kiểm thử giữa skill 09 (check result) và skill 10 (test report). Hoạt động với bất kỳ Jira project nào — project key đọc từ `qa-config.yaml` của từng dự án:

```
09-check-result → [bug confirmed] → 09b-log-defect → Jira Defect created → 10-test-report
```

**Trigger keywords:** `log bug`, `tao defect`, `tao bug jira`, `push defect`, `report loi`, `bug confirmed`

**L2 governance gate** — AI emit confirm block trước khi gọi script. User phải nhập "yes" mới tạo ticket.

**Auto features:**
- Sprint tự detect từ `customfield_10018` của US issue (board 10707 → 10710 fallback)
- Tự link `Defect → US` (Relates to)
- Severity mapping: S1→Critical/Highest, S2→Major/High, S3→Normal/Medium, S4→Minor/Low

### jira_create_bug.py — Tham số chính
...
Xem đầy đủ 18 tham số trong `README.md` §Jira Tools → jira_create_bug.py.

### Upgrade guide từ v2.1 → v2.2

1. **Không có breaking changes** — tất cả skill trigger keywords giữ nguyên
2. **Skill 09b** là skill mới hoàn toàn — không ảnh hưởng skill 09 cũ
3. **template/** đã dọn sạch `.json` sidecars — nếu ai đang dùng các file `.json` này để push Confluence thì cần dùng `tools/confluence_md.py --file <md> --parent-id <id>` thay thế
4. **tools/.env** đã bị xóa khỏi repo — tạo lại file `.env` local theo hướng dẫn trong README §Setup credentials
