...
description: >
  Execute smoke tests on production after deploy: quickly verify core business flows,
  confirm the deploy succeeded, or trigger rollback. Trigger: smoke test
  production, post-deploy smoke, check production, verify deploy, post-deploy check.
