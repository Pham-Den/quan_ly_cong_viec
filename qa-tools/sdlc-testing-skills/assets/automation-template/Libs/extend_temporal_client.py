...
class TemporalClient:
    ...
    TEMPORAL_URL = os.environ.get("TEMPORAL_URL", "https://your-temporal.example.com")

    # def __init__(self)-> None:
    #     logging.info("worrk")

    @keyword("Get Workflow Info")
    def get_workflow_info(self, query_type="WorkflowId", operator="Equals", value=None,  namespace="your-service-namespace"):
        """
        Gửi GET request để lấy thông tin workflow từ Temporal API.

        Args:
            namespace (str): Namespace (VD: "your-service-qc" - mặc định là your-service)
    ...
    def get_list_schedule_temporal(self, schedules="GenerateSessionScheduleWorkflowImpl", namespace="your-service-namespace"):
        env = BuiltIn().get_variable_value('${env}', 'uat').lower()
        # env = "qc"
        namespace = f"{namespace}-{env}"
    ...
    def get_settlement_schedules(self, schedules="GenerateSessionScheduleWorkflowImpl"):
        try:
            jsondata = self.get_list_schedule_temporal(schedules, 'your-service')
