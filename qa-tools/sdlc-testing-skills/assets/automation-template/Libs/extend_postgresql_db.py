...
@library
class ExtendPostgreSQLDB:
    """Library for querying PostgreSQL databases (DEV, UAT, QC)"""
    ...
    def __init__(self) -> None:
        logger.debug("Initializing ExtendPostgreSQLDB")
        ...
        try:
            ...
            with open(cred_file, 'r', encoding='utf-8') as f:
                creds = json.load(f)
                self.your_db_user_creds = creds.get("your_db_user", {})
    ...
    def _get_db_config(self, env: str) -> Dict[str, Any]:
        ...
        env = env.lower()
        db_user_key = f"your_db_user_{env}"
        ...
        return {
            ...
            'dbname': db_user_key,
            'user': db_user_key,
            'password': self.your_db_user_creds.get(db_user_key, ''),
        }
    ...
    @keyword('Postgres Query Database')
    def postgres_query_database(self, sql_query: str):
        """
        Truy vấn cơ sở dữ liệu your_db_user trong PostgreSQL.
