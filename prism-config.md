...
```yaml
prism:
  version: "2.2.4"  # installed PRISM version — set by setup.sh / setup.sh --upgrade
...
sprints:
  - id: v1
    ...
    # kind: reflect            # OPTIONAL (brownfield onboarding). A "reflect" sprint records EXISTING
    #                          #   code as `reflected` items only (no new build). Build/spec gates are
    #                          #   skipped, but it STILL seals into Living Truth via the normal seal.
    #                          #   Absent / any other value = normal full-flow sprint (greenfield default).
    #                          #   Definition + rules: core/sprint-manager.md § Reflect Sprint.

# Atomic ID counters (22 prefixes — the 21 v2.0.0 model prefixes + sprint-independent BUG for the ORIGIN-1 bug lane).
# Read + bumped by `core/tools/get_next_id.py --type {EP|FR|US|AC|BR|NFR|TC|SCREEN|DS-COMP|ARCH-COMP|ARCH|GLOSS|PERSONA|MR|SEQ|ENT|ADR|FLOW|API|EVT|PR|BUG}`.
