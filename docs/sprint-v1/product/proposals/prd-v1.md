...
Dữ liệu tracking không lưu credential hoặc secret raw. Execution input/output được mask theo `BR-001` trước khi lưu hoặc hiển thị.
...
#### 12. Industry Checklist Decisions
...
**Host:** `ACTIVE -> INACTIVE` khi người dùng vô hiệu hóa; transition này chuyển workflow liên quan sang DISABLED. `INACTIVE -> ACTIVE` khi kích hoạt lại, nhưng workflow vẫn DISABLED theo luồng `Host ACTIVE -> user review -> validation sạch -> user enable -> READY`. `ACTIVE|INACTIVE -> DELETED` chỉ sau cảnh báo dependency và xác nhận; `DELETED -> ACTIVE` không hợp lệ trong v1.

**Workflow:** `DRAFT -> READY` sau validation sạch; `DRAFT|READY -> DISABLED` khi dependency mất, API đổi HTTP method, Host chuyển INACTIVE/DELETED hoặc người dùng vô hiệu hóa. `DISABLED -> READY` chỉ sau khi dependency/Host hợp lệ, người dùng review, validation sạch và chủ động enable; Host ACTIVE không tự gây transition này. `READY -> RUNNING` khi người dùng chạy latest saved version; `RUNNING -> SUCCEEDED` khi mọi step thành công; `RUNNING -> FAILED` khi một step hết retry. `DRAFT|DISABLED -> RUNNING` và `SUCCEEDED|FAILED -> RUNNING` trên cùng execution là không hợp lệ; rerun tạo execution mới bằng latest version.

**Execution:** Khi `PENDING -> RUNNING`, hệ thống cố định latest workflow version và snapshot environment; hai snapshot không đổi trong execution. `RUNNING -> SUCCEEDED` khi tất cả step thành công; `RUNNING -> FAILED` khi step hiện tại hết retry hoặc gặp lỗi không retryable. Execution terminal không được đổi trạng thái; chạy lại tạo record mới và giữ liên kết với execution nguồn.
