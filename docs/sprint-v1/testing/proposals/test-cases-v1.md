...
# Test Cases Proposal — API Lab Workflow Orchestration (Sprint v1)

This proposal is the canonical sprint Test intent. Runnable repo tests remain owned by Plan/Implement; generated TSV files are deterministic views of this Markdown.

## New

### Conventions, Priority Levels And Traceable Sources

| Contract | Definition |
|---|---|
| Priority | `P0` blocks sprint acceptance; all cases in v1 are P0 because they cover Must scope, security, integrity, recovery or a release gate. |
| Status tag | `planned-automated` means Implement must deliver runnable repo evidence; `planned-manual` requires named human checklist/evidence. |
| Technique title | The final bracket before the status tag names the applied technique: Positive, Negative, BVA, EP, DT, ST, DD, or a `+` combination. |
| Assertions | Every case must name fixture identity, exact API/screen action, observable outcome, mutation/provider-call invariant and reset rule. |
| AC-001–043 source | [EP-001 base proposal](../../product/proposals/epics/EP-001-api-lab-workflow-orchestration-v1.md), matching stable `AC-NNN` anchor. |
| AC-044–059 source | [Approved product delta v1.7.18-api-lab-undo-warning-viewport](../../changes/v1.7.18-api-lab-undo-warning-viewport/product-delta-v1.7.18-api-lab-undo-warning-viewport.md), matching stable `AC-NNN` anchor. |
| Architecture source | Approved proposal set under [architecture/proposals](../../architecture/proposals/) using the cited API/NFR/ADR/SEQ/FLOW stable ID. |
| Design source | Approved [design-system-v1.md](../../design/proposals/design-system-v1.md) using the cited SCREEN/DS-COMP stable ID. |
...
| [Derived] AuthZ boundary | Architecture effective truth §API-001–023 + ADR-006 + EXC-AUTH-001 | Callback state/PKCE/MFA, protected UI/direct API, CSRF/revocation/idle/IAM fail-closed and no browser token storage | TC-065, TC-068 | covered | — |
| [Derived] Dependency failure | Architecture effective truth §SEQ-001–006 + FLOW-001/002 | IAM/key/DB/provider timeout, error and partition behavior | TC-062 | covered | — |
| [Derived] Idempotency / data integrity | Architecture effective truth §API writes + ADR-004 | No duplicate side effect and atomic final state | TC-060, TC-063, TC-075 | covered | — |
...
| [Derived] Contract compatibility | Architecture effective truth §Versioned API-001–023 consumers + PR-008 + EXC-STACK-001 | OpenAPI/error compatibility, import boundaries, supported-version/EOL and no production source map | TC-061, TC-073 | covered | — |
...
| [Derived] Configured-parameter enforcement | Architecture effective truth §Timeout/retry/capacity/retention config + EXC-RETRY-001 | Provider 0–5 fixed-delay/full allowlist/current-Step retry plus non-provider max-three exponential full jitter | TC-008, TC-011, TC-022–024, TC-060, TC-062, TC-067, TC-071 | covered | — |
| [Derived] Integration routing | Architecture effective truth §Gateway/IAM/key/audit mandatory ports | No bypass around Identity/Gateway/Audit boundaries | TC-062, TC-065, TC-068 | covered | — |
...
| Security/Observability | TC-064/065/068/070 | BR-001/008; EXC-AUTH-001 | covered | TC-068 | TC-065/068 | TC-065/068 | TC-062 | TC-061 | covered | TC-065/068 | callback/state/PKCE/MFA and lifecycle; authorized non-production only |
...
<!-- ID: TC-065 -->
<!-- VERIFIES: NFR-004 -->
### TC-065: [NFR-004][Security+SIT] [EXC-AUTH-001] OIDC callback state, PKCE, MFA and protected paths enforce one fail-closed browser boundary `[planned-automated]` `P0`

**Area**: SEC
**Traceability**: FR-001–012, NFR-004, EXC-AUTH-001
**Design states referenced**: SCREEN-001–008 authenticated full-page/workspace surfaces; exact visual states are covered by TC-072
**API / NFR refs**: API-001–023, especially API-017–020; ADR-006; PR-002; EXC-AUTH-001
**Manual / Auto boundary**: automated
**Test Level**: integration
**Test Type**: SIT
**Export target**: sit
**Smoke**: N
**Environment**: Local Docker Compose and CI
**Data needs**: Synthetic active/expired/revoked/uncertain identities; one-time state records; PKCE verifier/challenge pairs; MFA-sufficient/insufficient `amr`/`acr` claims; isolated ephemeral schema; no production PII.
**Teardown / reset**: Reset fixture schema, fake dependency state and controllable clock after the case.
**Depends on**: Plan TG-02 OIDC/session/CSRF implementation and TG-26 auth security harness with Central-IAM fake.
**Automation intent**: Auto=Y; future repo suite in the Plan-owned test delta using Architecture-approved frameworks and contracts.
**External QA handoff needs**: N/A — no external QA in sprint v1.
**Owner of execution context**: khanh-pham; local execution and CI evidence retained in repository artifacts.
**Systems involved**: Browser OIDC initiation/callback, Central-IAM fake, server session, Fastify identity hooks, application handlers
**System A expected**: Unauthenticated, expired, revoked and uncertain sessions are denied and return URL is preserved where allowed.
**System B expected**: No protected handler mutates or returns payload when identity enforcement rejects.
**Screen under test**: SCREEN-001–008 protected routes and workspace surfaces; this case verifies their shared authentication boundary, while TC-072 owns exact visual-state observation

**Given**:
- Active, expired, revoked, inactive and IAM-uncertain synthetic identities; valid CSRF only for active actor.
- API-017 creates one server-side state record bound to return URL and a PKCE S256 challenge. Callback rows include correct, missing, mismatched, expired and replayed state; correct/missing/wrong verifier; and MFA-sufficient/insufficient `amr`/`acr` claims.

**When**:
- Execute API-017→API-018 for every state/PKCE/MFA row, replay the successful callback, then invoke protected actions through UI and direct API including method/path tampering.

**Then**:
- [ ] Only the callback with the bound unexpired one-time state, matching PKCE verifier and required MFA assurance creates a Secure/HttpOnly/SameSite server session and restores the validated return URL.
- [ ] Missing/mismatched/expired/replayed state, missing/wrong PKCE verifier, or insufficient MFA assurance is rejected with the documented typed outcome; no session/domain mutation is created and the state cannot be reused.
- [ ] Only active authenticated actor succeeds.
- [ ] Every bypass/direct path is denied before domain mutation.
- [ ] Session cookie is Secure/HttpOnly/SameSite and no bearer token persists in browser storage.

**Test Data**: `state=valid,missing,mismatch,expired,replayed; pkce=S256_valid,missing,wrong; mfa=required_sufficient,missing_or_insufficient; session_created=valid_row_only; browser_storage_token_count=0; synthetic=true; isolation=ephemeral; reset=required`
...
<!-- ID: TC-070 -->
<!-- VERIFIES: NFR-006 -->
### TC-070: [NFR-006][SIT] Correlation, audit, alerts and exporter degradation meet exact observability bounds `[planned-automated]` `P0`

**Area**: OBS
**Traceability**: NFR-006, FR-005, FR-008
**Design states referenced**: N/A — non-UI architecture obligation
**API / NFR refs**: FLOW-001/002; ARCH-COMP-007; NFR-006
**Manual / Auto boundary**: automated
**Test Level**: integration
**Test Type**: SIT
**Export target**: sit
**Smoke**: N
**Environment**: Local Docker Compose and CI with exporter/alert fakes and controllable retention clock
**Data needs**: Known 64-character request ID, trace ID, execution ID and synthetic secret markers.
**Teardown / reset**: Reset fixture schema, fake dependency state and controllable clock after the case.
**Depends on**: —
**Automation intent**: Auto=Y; Implement delivers runnable evidence using approved frameworks/contracts.
**External QA handoff needs**: N/A — no external QA in sprint v1.
...
**Given**:
- Alert fixtures cover job oldest age at 60s and >60s sustained 5m; dead jobs 0/1; retention lag 48h/>48h; error rate 5%/>5% sustained 5m; active-count invariant match/mismatch.
- Audit clock covers online 6 months and retained 12 months; exporter is unavailable for 24h.
- Ingress fixtures send a missing, valid 64-character, overlength 65-character, malformed and duplicate `X-Request-ID`, plus a trusted generated and direct untrusted `X-Trace-ID`; the flow crosses response, downstream HTTP, job and worker boundaries.
- Offline Sentry fixtures enqueue 100 and 101 redacted envelopes aged 23h59m59s, 24h and 24h+1s; envelope variants include or omit `app_version`, immutable `build_id`, session ID, masked user ID, device/OS/browser/viewport/network class and 10 redacted breadcrumbs.
- Release fixtures contain a verified source-map upload or a missing upload, remove uploaded maps from the public bundle, and keep the upload credential absent from runtime/browser configuration.

**When**:
- Run success/failure flows, disable exporter, drive every alert threshold at below/equal/above values and advance audit clock.
- Exercise every request/trace-ID vector and inspect the response echo, downstream HTTP headers, job payload, worker context and structured logs; verify neither identifier participates in authentication or idempotency decisions.
- Enqueue offline envelopes at the 100/101 boundary, advance the controllable clock through the TTL boundary, reconnect, emit each approved frontend error category and LCP/INP/CLS, and inspect the local breadcrumb ring.
- Query by request/trace/execution ID immediately and after 5 minutes; execute the source-map promotion gate and scan envelopes, bundles and runtime configuration for PII/secrets.

**Then**:
- [ ] A valid request ID of ≤64 characters is retained; missing, malformed, duplicate or 65-character values are replaced by one server-generated ID; direct untrusted trace IDs are replaced by the trusted telemetry context.
- [ ] Responses echo the accepted/generated request and trace IDs; downstream HTTP, job, worker, provider and structured-log evidence propagates the same pair and is searchable within 5 minutes, without using either identifier for authorization or idempotency.
- [ ] Structured logs include `timestamp,level,service,environment,trace_id,request_id,user_id,operation,message,duration_ms,http_status,error_code,error_message`; correlation coverage is 100%.
- [ ] Alerts do not fire at exact strict boundaries; they fire for oldest job >60s sustained 5m, dead job >0, retention lag >48h, error rate >5% sustained 5m and any active-count invariant mismatch.
- [ ] Offline queue size 100 is accepted; the 101st envelope is dropped with an overflow metric; an item at 23h59m59s retries on reconnect, while items aged ≥24h expire and increment the expiry/drop metric without rolling back business data.
- [ ] Every accepted envelope has the required app/build/session/masked-user/device/OS/browser/viewport/network context and at most the latest 10 redacted breadcrumbs; `sendDefaultPii=false`, raw secret/body/email/phone scans are zero, and LCP/INP/CLS plus approved error categories are emitted.
- [ ] Promotion blocks if verified source maps are not uploaded; after upload maps are absent from the public bundle and upload credentials are absent from runtime/browser configuration.
- [ ] Audit stays append-only, online 6 months and retained 12 months; secret scan remains zero.

**Test Data**:
- `request_id=missing,valid64,overlength65,malformed,duplicate; trace_id=trusted,untrusted; propagation=response,http,job,worker,provider,log; correlation=100%; searchable<=5m`
- `frontend_queue=100,101; envelope_age=23h59m59s,24h,24h+1s; queue_ttl=24h; sendDefaultPii=false; breadcrumbs=10,11; perf=LCP,INP,CLS; context=required_complete,missing_field`
- `audit=online6m/retained12m; alerts=job_age>60s/5m,dead>0,retention_lag>48h,error>5%/5m,active_mismatch; exporter_alert<=5m; source_map=verified,missing; synthetic=true; reset=required`
---
<!-- ID: TC-071 -->
<!-- VERIFIES: NFR-007 -->
...
<!-- ID: TC-072 -->
<!-- VERIFIES: NFR-008 -->
### TC-072: [NFR-008][Accessibility+Regression] Eight screens expose every approved state, copy, CTA, exit and viewport behavior `[planned-manual]` `P0`

**Area**: A11Y
**Traceability**: NFR-008, FR-001–012
**Design states referenced**: SCREEN-001–008 all state identifiers below
**API / NFR refs**: SCREEN-001–008; DS-COMP-001–011; NFR-008
**Manual / Auto boundary**: manual visual/keyboard/screen-reader judgment; axe/Playwright supplies supporting evidence
**Test Level**: e2e
...
**Automation intent**: Auto=N for human judgment; TG-28 links axe, screenshots and focus-event evidence while consuming TG-24 browser fixtures and TG-22 reset.
**External QA handoff needs**: N/A — no external QA in sprint v1.
...
**Then**:
- [ ] All 19 target rows and every scenario key execute independently; no row relies on prior state.
...
**Test Data**:
- `fixture_source=TG-24 tests/e2e/api-lab-workflow/** + frontend/tests/integration/**; harness=TG-28 tests/accessibility/** + tests/e2e/viewport/**; reset=TG-22 scripts/reset.* + browser-harness reset; screens=8; components=11; target_rows=19; browsers=Chromium,Firefox; widths=1279,1280,1440; zoom=100%,200%; synthetic=true`
...
- [x] DOC-1/2/3, LINK-1/2, ORB-1 and TEST-1–8 are represented.
- [x] All 59 effective-truth ACs and BR-001–012 map to concrete TC IDs.
- [x] Every Must FR has multiple positive/error/edge cases and at least one TEST-2 executable case.
- [x] All ten triggered architecture-derived obligation types map to cases.
- [x] Every AC technique cell has a concrete Y/N rationale and each Y tag appears in its TC heading.
- [x] Functional, SIT, NFR, data, automation and external-QA boundaries are explicit.
- [x] Every TC uses synthetic isolated data and owns reset/teardown expectations.
- [x] No runtime dataset, repo test implementation, secret, or external automation pack is created here.
- [x] Summary counts cover TC-001–TC-076; generated TSV companions are regenerated from this file.
