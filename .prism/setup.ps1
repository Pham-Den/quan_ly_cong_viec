...
function Remove-ManifestTree([string]$ToolRoot, [string]$Label) {
    if (-not (Test-Path -LiteralPath $ToolRoot -PathType Container)) { return }

    $Manifest = Join-Path $ToolRoot "MANIFEST.txt"
    if (-not (Test-Path -LiteralPath $Manifest -PathType Leaf)) {
        Write-Warn "$Label manifest not found; skipping stale $Label file cleanup"
        return
    }

    foreach ($GeneratedDir in @("node_modules", "dist", ".docs-view", "docs-dist")) {
        $GeneratedPath = Join-Path $ToolRoot $GeneratedDir
    ...
    $Base = $ToolRoot.TrimEnd("\", "/")
    $PrunedCount = 0
    Get-ChildItem -LiteralPath $ToolRoot -File -Recurse -Force -ErrorAction SilentlyContinue | ForEach-Object {
        $RelativePath = $_.FullName.Substring($Base.Length).TrimStart("\", "/").Replace("\", "/")
        if (-not $Managed.ContainsKey($RelativePath)) {
            Remove-Item -LiteralPath $_.FullName -Force
            $PrunedCount += 1
        }
    }

    Get-ChildItem -LiteralPath $ToolRoot -Directory -Recurse -Force -ErrorAction SilentlyContinue |
        Sort-Object { $_.FullName.Length } -Descending |
        ForEach-Object {
            if (-not (Get-ChildItem -LiteralPath $_.FullName -Force -ErrorAction SilentlyContinue)) {
                Remove-Item -LiteralPath $_.FullName -Force
    ...
    if ($PrunedCount -gt 0) {
        Write-Ok "Removed $PrunedCount stale managed $Label file(s)"
    } else {
        Write-Ok "No stale managed $Label files found"
    }
}
...
if ($Action -eq "upgrade") {
    ...
    Remove-ManifestTree (Join-Path (Join-Path $ProjectRoot "tools") "docs-viewer") "docs-viewer"
