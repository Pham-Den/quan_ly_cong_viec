...
prune_manifest_tree() {
    TOOL_ROOT="$1"
    LABEL="$2"
    MANIFEST="$TOOL_ROOT/MANIFEST.txt"
    ...
    if [ ! -f "$MANIFEST" ]; then
        warn "$LABEL manifest not found; skipping stale $LABEL file cleanup"
        return
    fi

    for generated_dir in node_modules dist .docs-view docs-dist; do
        if [ -e "$TOOL_ROOT/$generated_dir" ]; then
            rm -rf "$TOOL_ROOT/$generated_dir"
        fi
    done

    PRUNED_LIST="${TMPDIR:-/tmp}/prism-${LABEL}-pruned-$$.txt"
    : > "$PRUNED_LIST"

    find "$TOOL_ROOT" -type f | while IFS= read -r file; do
        rel="${file#"$TOOL_ROOT"/}"
        if ! grep -Fx -- "$rel" "$MANIFEST" >/dev/null 2>&1; then
            rm -f "$file"
            printf '%s\n' "$rel" >> "$PRUNED_LIST"
        fi
    done
    ...
    PRUNED_COUNT=$(wc -l < "$PRUNED_LIST" | tr -d ' ')
    if [ "$PRUNED_COUNT" -gt 0 ]; then
        ok "Removed $PRUNED_COUNT stale managed $LABEL file(s)"
    else
        ok "No stale managed $LABEL files found"
    fi
    rm -f "$PRUNED_LIST"

    find "$TOOL_ROOT" -mindepth 1 -depth -type d -empty -exec rmdir {} \; 2>/dev/null || true
}
...
if [ "$ACTION" = "upgrade" ]; then
    ...
    prune_stale_package_files
    prune_manifest_tree "$PROJECT_ROOT/tools/docs-viewer" "docs-viewer"

    # --- Step 4: Detect platforms from existing adapters ---
    # On upgrade we refresh whatever is already installed. The meta-keys "all"
    # and "core" (the default when no --platform is given) both mean auto-detect
    # — never route them straight to adapter_src, which only knows real platform
    # keys (passing "core" there returns empty and, under `set -e`, aborts the
    # upgrade after backup but before the adapters/config are rewritten).
    if [ "$PLATFORM" = "all" ] || [ "$PLATFORM" = "core" ]; then
