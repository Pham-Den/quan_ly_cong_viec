...
> **Reflect sprint (`kind: reflect` — `core/sprint-manager.md § Reflect Sprint`):** `DES-1`/`DES-2` (4 states, exact error copy, state identifiers) are **exempt** — the existing UI is *observed* (reflected), captured by description + screenshot, not a full spec. A backend slice with no UI declares **Design-N/A** — a `DESIGN-OVERVIEW-001` block noting "no UI surface" that carries an explicit `<!-- COVERAGE: N/A -->` marker, so the deterministic `LTV-DESIGN-COV` gate recognises the intentional opt-out (an empty Design-to-FR map WITHOUT that marker is treated as an unfilled map, i.e. a coverage gap, not a free pass). The `validate design` audit that writes the active validate file applies these **same** exemptions — it must not raise a `DES-1`/`DES-2` blocker for a reflected UI, otherwise the "active validate design file clean" bullet below would silently re-impose them. `validate_proposal.py` structural checks and `LTV-DESIGN-COV` STILL apply.
> **DOC-3 intent-bearing design sections in reflect:** record what the UI *observably does* — screens, states, components, copy rendered, and the accessibility behavior actually present (alt text, ARIA, keyboard nav) — those are code/UI-derivable and STILL hold. Only the WHY a screenshot cannot supply defers: the *committed* accessibility target / WCAG level (§11), brand / design-principle *rationale* (§1), user-flow *intent* → satisfied by `N/A — reflected, intent deferred`, never left blank (blank = `DOC-3` blocker) and never fabricated. The deferred WHY parks as a `HOÃN` Open Question in **PRD §11** — the design template has no open-questions ledger of its own, and this UI/UX WHY is product-owned (home-selection per `core/sprint-manager.md § Reflect Sprint` rule 8 "WHY-gap deferral"). Forward (non-reflect) sprints keep these sections mandatory and ASK the user.
...
- `python .prism/core/tools/validate_living_truth.py --effective --sprint v{X} --candidate-phase design` returns any blocker — a Must Have FR (product effective truth) missing from the merged `DESIGN-OVERVIEW-001` Design-to-FR Traceability map (`LTV-DESIGN-COV`, the `DESIGN-1` deterministic backstop). This runs the coverage check on the pre-seal effective truth so drift is caught at approve, not deferred to seal. `--candidate-phase design` is REQUIRED here for the same reason as architecture: the gate runs while the design proposal is still `DRAFT` (status flips to `APPROVED` only after the gate passes), so the flag composes that DRAFT proposal as the approval candidate and answers "if I approve this draft, is every Must FR traced?" — without it the check would compose approved-only content, never see the trace just authored in the DRAFT, and deadlock. For a change-pack approval (`approve changes`) keep `--candidate-phase design` AND add `--candidate-pack v{X}.{Y}.{Z}-{slug}` (the pack folder name): `--candidate-phase design --candidate-pack v{X}.{Y}.{Z}-{slug}`. The `--candidate-phase design` is REQUIRED even for a change pack — it selects which coverage runs; `--candidate-pack` alone defaults to architecture and would silently skip design coverage (the tool now refuses `--candidate-pack` without `--candidate-phase`). Findings stay hard blockers; the candidate is scoped to the exact sprint/pack (no unrelated draft, and no pack from another sprint, is pulled in). An empty / unfilled `DESIGN-OVERVIEW-001` Design-to-FR map does NOT pass: either trace every Must FR, or — for a genuinely UI-less scope — declare it explicitly with a `<!-- COVERAGE: N/A -->` marker in the overview block (the only way an FR-ref-free map passes; a merely-empty map is a coverage gap). (For a reflect sprint, `LTV-DESIGN-COV` still applies per the reflect note above.)
...
- `VAL-1`: the active validate file records structural coverage and rule coverage for `DOC-1`, `DOC-2`, `DOC-3`, `LINK-1`, `LINK-2`, `ORB-1`, `SEM-1`, `DES-1`, and `DES-2`.
- `SEM-1`: run the semantic integrity pass across design effective truth, approved Product, and the current design proposal: catch screens / flows / states that contradict FR / US / AC intent, duplicated screens or components under different IDs, terminology or copy drift from Product / glossary, stale Product assumptions after approval or change packs, and redundant or out-of-scope UI behavior.
