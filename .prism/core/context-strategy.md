...
**Working sprint detection** (per `core/sprint-manager.md § Cross-Sprint Routing`): new work defaults to the newest open (`sealed: false`) sprint; continuing existing work uses the open sprint with the most recently updated DRAFT file (by `updated` frontmatter; ties → higher sprint number). User can be explicit: `resume design in sprint-v2`.
