...
Rules:
  ...
  VP-13 (--check-c4) Architecture overview proposal that CLAIMS a C4 model must reference
        a C4 Draw.io/XML source that exists and covers the 3 required levels (System
        Context, Container, Component), each non-empty. Opt-in: off by default so seal's
        pre-flight and non-C4 callers are unaffected; the architecture validate/approve
        gate passes --check-c4. The 4th C4 level (Code) is not required.
  VP-14 (--check-dfd) Data-flow proposal that CLAIMS a DFD must reference a DFD Draw.io/XML
        source that exists and holds at least one non-empty diagram page (ARCH-2 needs >= 1
        DFD); pages with no arrows or with unlabeled arrows raise warnings. Opt-in, same
        rationale as VP-13.
  VP-15 Sequence proposal SEQ items must include a fenced Mermaid sequenceDiagram block.
...
from validate_arch_assets import (  # noqa: E402
    ASSET_DRAWIO_REF_RE,
    evaluate_c4_pages,
    evaluate_dfd_pages,
    parse_drawio_pages,
...
# VP-13 (opt-in via --check-c4): the architecture overview proposal and a heading that
# claims a C4 model. The C4 asset check only fires when the proposal actually CLAIMS C4
# (a `assets/c4*.drawio` reference or a C4 section), so non-C4 / minimal fixtures stay
# unaffected and the "C4 entirely absent" case is left to the LLM-driven validate + DOC-3.
ARCH_OVERVIEW_FILE_RE = re.compile(r"^architecture-v\d+\.md$")
C4_SECTION_HEADING_RE = re.compile(r"(?im)^#{1,6}\s+.*\bC4\b")

# VP-14 (opt-in via --check-dfd): the data-flow proposal and a heading that claims a DFD.
DATA_FLOW_FILE_RE = re.compile(r"^data-flow-v\d+\.md$")
DFD_SECTION_HEADING_RE = re.compile(r"(?im)^#{1,6}\s+.*\bDFD\b")
...
class DiagramRequirement:
    ...
    prefix: str
    fence_language: str
    diagram_type_re: re.Pattern[str]
    diagram_type_label: str
    invalid_substitute_label: str


DIAGRAM_REQUIREMENTS_BY_PREFIX = {
    "SEQ": DiagramRequirement(
        rule="VP-15",
        prefix="SEQ",
        fence_language="mermaid",
        diagram_type_re=re.compile(r"^\s*sequenceDiagram\s*(%%.*)?$"),
        diagram_type_label="`sequenceDiagram`",
        invalid_substitute_label="ASCII/plaintext ladder diagrams or prose-only sequence descriptions",
    ),
}
...
def validate_epic_split_content_ownership(
    ...
    living_us_to_epic = discover_living_us_to_epic(
        project_root if project_root is not None else project_root_from_source_path(source_path)
    )
...
def _diagram_fence_start_re(fence_language: str) -> re.Pattern[str]:
    return re.compile(rf"^\s*(```+|~~~+)\s*{re.escape(fence_language)}\b.*$", re.IGNORECASE)


# Mermaid lets blank lines, `%%` comments / `%%{init}%%` directives, and a leading
# `---` YAML frontmatter block precede the diagram-type keyword. Matching the type
# anywhere in the body would wrongly pass a `flowchart` whose node text merely starts
# with `sequenceDiagram`, so we test the FIRST significant (type-declaring) line only.
_MERMAID_SKIP_RE = re.compile(r"^\s*(%%.*)?$")


def _first_significant_diagram_line(body_lines: list[str]) -> str | None:
    i, n = 0, len(body_lines)
    while i < n and _MERMAID_SKIP_RE.match(body_lines[i]):
        i += 1
    if i < n and body_lines[i].strip() == "---":  # skip a leading `--- ... ---` block
        i += 1
        ...
        while i < n and _MERMAID_SKIP_RE.match(body_lines[i]):
            i += 1
    return body_lines[i] if i < n else None


def _is_ascii_art_line(line: str) -> bool:
    """True for a non-blank line built only from box-drawing characters (pipes, dashes,
    plus) and whitespace — pasted ASCII ladder art, never valid Mermaid statement syntax.
    Lets a real `sequenceDiagram` header followed by a hand-drawn ladder still be rejected."""
    return "|" in line and line.strip() != "" and all(ch in "|-+ \t" for ch in line)


def _has_required_fenced_diagram(
    block_lines: list[str],
    requirement: DiagramRequirement,
) -> bool:
    """Return True when a block has a closed fence whose first diagram-type line is the
    required type (not merely a later body line that mentions it) and whose body is not
    pasted ASCII ladder art under that header."""
    fence_start_re = _diagram_fence_start_re(requirement.fence_language)
    in_required_fence = False
    fence_char: str | None = None
    fenced_body: list[str] = []

    for line in block_lines:
        if in_required_fence:
            fence_m = CODE_FENCE_RE.match(line.lstrip())
            if fence_m and fence_m.group(1)[0] == fence_char:
                first = _first_significant_diagram_line(fenced_body)
                if (
                    first is not None
                    and requirement.diagram_type_re.match(first)
                    and not any(_is_ascii_art_line(b) for b in fenced_body)
                ):
                    return True
                in_required_fence = False
                fence_char = None
                fenced_body = []
                continue
            fenced_body.append(line)
            continue

        fence_m = fence_start_re.match(line)
        if fence_m:
            in_required_fence = True
...
def validate_required_diagram_blocks(
    prop_sections: dict[str, list[tuple[str, list[str]]]],
    phase: str | None,
    report: Report,
) -> None:
    """Diagram-like artifacts must use their configured executable diagram fence."""
    ...
    for section_name, items in prop_sections.items():
        if section_name == "Removed":
            continue
        for aid, block_lines in items:
            requirement = DIAGRAM_REQUIREMENTS_BY_PREFIX.get(_prefix_of(aid) or "")
            if requirement is None:
                continue
            if _has_required_fenced_diagram(block_lines, requirement):
                continue
            report.add_blocker(
                requirement.rule,
                f"{section_name}/{aid}: {requirement.prefix} items must contain a fenced "
                f"{requirement.fence_language.title()} code block with "
                f"{requirement.diagram_type_label} as the diagram type. "
                f"{requirement.invalid_substitute_label} are not valid.",
            )
...
def validate_c4_assets(
    source_path: Path | None, phase: str | None, body: str, report: Report
) -> None:
    """VP-13 — deterministic C4 Draw.io/XML structure check (opt-in).

    Scoped to the architecture overview proposal (`architecture-v{X}.md`) and fired only
    when that proposal CLAIMS a C4 model: it references a C4 Draw.io source
    (`assets/c4*.drawio`) or carries a C4 section heading. When it claims C4, the
    referenced source must exist and cover the 3 required levels (System Context,
    Container, Component), each non-empty. The 4th C4 level (Code / L4) is not required.

    A proposal that does not claim C4 stays silent (keeps minimal / non-C4 fixtures green);
    the "C4 entirely absent" case is owned by the LLM-driven validate + DOC-3, not here.
    """
    if source_path is None or phase != "architecture":
        return
    if not ARCH_OVERVIEW_FILE_RE.match(source_path.name):
        return

    drawio_refs = list(dict.fromkeys(ASSET_DRAWIO_REF_RE.findall(body)))  # de-dup, keep order
    c4_refs = [ref for ref in drawio_refs if "c4" in Path(ref).name.lower()]
    claims_c4 = bool(c4_refs) or bool(C4_SECTION_HEADING_RE.search(body))
    if not claims_c4:
        return  # proposal does not claim a C4 model — nothing to assert

    if not c4_refs:
        report.add_blocker(
            "VP-13",
            "architecture proposal has a C4 section but references no C4 Draw.io/XML "
            "source (expected an `assets/c4-*.drawio` reference)",
        )
        return

    c4_rel = c4_refs[0]
    # The proposal writes `assets/...` relative to the post-seal Living Truth location
    # (`docs/architecture/`), so it resolves against the architecture dir — the parent of
    # `proposals/` — not the proposal's own directory.
    c4_path = source_path.parent.parent / "assets" / c4_rel
    if not c4_path.is_file():
        report.add_blocker(
            "VP-13",
            f"C4 Draw.io/XML source `assets/{c4_rel}` is referenced but the file does not "
            f"exist at {c4_path}",
        )
        return

    try:
        pages = parse_drawio_pages(c4_path.read_text(encoding="utf-8"))
    except OSError as e:
        report.add_blocker("VP-13", f"cannot read C4 Draw.io/XML source `assets/{c4_rel}`: {e}")
        return

    for level, _rule, msg in evaluate_c4_pages(pages, rule="VP-13"):
        if level == "blocker":
            report.add_blocker("VP-13", msg)
        elif level == "warn":
            report.add_warn("VP-13", msg)
        # 'info' is dropped — validate_proposal output is finding-only.


def validate_dfd_assets(
    source_path: Path | None, phase: str | None, body: str, report: Report
) -> None:
    """VP-14 — deterministic DFD Draw.io/XML structure check (opt-in via --check-dfd).

    Scoped to the data-flow proposal (`data-flow-v{X}.md`) and fired only when it CLAIMS a
    DFD: it references a DFD Draw.io source (`assets/...drawio`) or carries a DFD section
    heading. When it claims a DFD, every referenced source must exist and hold at least one
    non-empty diagram page (`ARCH-2` requires >= 1 DFD); pages with no arrows or with
    unlabeled arrows raise warnings. Notation shape-typing, "covers every meaningful data
    movement", user-group splitting, and connector routing stay LLM-judged in the report.
    """
    if source_path is None or phase != "architecture":
        return
    if not DATA_FLOW_FILE_RE.match(source_path.name):
        return

    drawio_refs = list(dict.fromkeys(ASSET_DRAWIO_REF_RE.findall(body)))  # de-dup, keep order
    # On the data-flow proposal every referenced Draw.io asset is a DFD source; prefer
    # `dfd-*`-named files but fall back to all drawio refs when none follow that convention.
    dfd_refs = [ref for ref in drawio_refs if "dfd" in Path(ref).name.lower()] or drawio_refs
    claims_dfd = bool(dfd_refs) or bool(DFD_SECTION_HEADING_RE.search(body))
    if not claims_dfd:
        return  # proposal does not claim a DFD — nothing to assert

    if not dfd_refs:
        report.add_blocker(
            "VP-14",
            "data-flow proposal claims a DFD but references no DFD Draw.io/XML source "
            "(expected an `assets/dfd-*.drawio` reference)",
        )
        return

    assets_dir = source_path.parent.parent / "assets"
    for rel in dfd_refs:
        dfd_path = assets_dir / rel
        if not dfd_path.is_file():
            report.add_blocker(
                "VP-14",
                f"DFD Draw.io/XML source `assets/{rel}` is referenced but the file does not "
                f"exist at {dfd_path}",
            )
            continue
        try:
            pages = parse_drawio_pages(dfd_path.read_text(encoding="utf-8"))
        except OSError as e:
            report.add_blocker("VP-14", f"cannot read DFD Draw.io/XML source `assets/{rel}`: {e}")
            continue
        for level, _rule, msg in evaluate_dfd_pages(pages, rule="VP-14"):
            if level == "blocker":
                report.add_blocker("VP-14", f"`assets/{rel}`: {msg}")
            elif level == "warn":
                report.add_warn("VP-14", f"`assets/{rel}`: {msg}")
            # 'info' is dropped — validate_proposal output is finding-only.
...
def validate(
    ...
    validate_epic_split_content_ownership(source_path, prop_sections, report, project_root)
    validate_testing_technique_tags(prop_sections, phase if isinstance(phase, str) else None, report)
    validate_required_diagram_blocks(prop_sections, phase if isinstance(phase, str) else None, report)
    if check_c4:
        validate_c4_assets(source_path, phase if isinstance(phase, str) else None, body, report)
    if check_dfd:
        validate_dfd_assets(source_path, phase if isinstance(phase, str) else None, body, report)
...
def main() -> int:
    ...
    parser.add_argument(
        "--project-root",
        help="Project root for Living-Truth cross-checks (default: derived from the --file path; "
        "accept it for uniform invocation with seal_sprint.py / validate_living_truth.py)",
    )
    parser.add_argument(
        "--against-living",
    ...
    parser.add_argument(
        "--check-c4",
        action="store_true",
        help="Also run VP-13: deterministic C4 Draw.io/XML structure check on the "
        "architecture overview proposal (3 required levels present + non-empty)",
    )
    parser.add_argument(
        "--check-dfd",
        action="store_true",
        help="Also run VP-14: deterministic DFD Draw.io/XML structure check on the "
        "data-flow proposal (>= 1 non-empty DFD; warns on missing / unlabeled arrows)",
    )
    ...
    report = validate(
        ...
        source_path=path,
        check_c4=args.check_c4,
        check_dfd=args.check_dfd,
