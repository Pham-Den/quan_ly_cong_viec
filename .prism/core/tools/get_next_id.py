...
Bug lane (ORIGIN-1):
  - 1 new prefix: BUG (sprint-independent defect record under docs/bugs/; no migration)

CLI:
  get_next_id.py --type {EP|FR|US|AC|BR|NFR|TC|SCREEN|DS-COMP|ARCH-COMP|ARCH|GLOSS|PERSONA|MR|SEQ|ENT|ADR|FLOW|API|EVT|PR|BUG} [--reserve N] [--config PATH]
...
def main() -> int:
    ...
    parser.add_argument(
        "--project-root",
        help="Project root containing prism-config.md (alternative to --config; for uniform "
        "invocation with seal_sprint.py / validate_living_truth.py)",
    )
    ...
    if args.config:
        ...
        config_path = Path(args.project_root) / "prism-config.md"
        if not config_path.is_file():
            sys.stderr.write(f"ERROR: config not found under --project-root: {config_path}\n")
            return 2
    else:
