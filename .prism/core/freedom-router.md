...
1. Locate candidate in-progress work across the project's docs — all sprint folders if the project uses sprints, or the flat docs if it is sprintless. Freedom follows the user's evident intent (no sprint-centric default); if intent is unclear, ask.
2. Scan those documents for the `updated` frontmatter field
