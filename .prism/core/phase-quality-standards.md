...
Single source of truth for the detailed PRISM quality bar used by `import [phase]`, every output-producing flow (`start`, `resume`, `continue`, `feedback:`, `start change:`, `start bug:`), and the user-invoked validate commands (`validate user story`, `validate design`, `validate architecture`, `validate plan`, `validate test`, `validate implementation --mode spec`, `validate implementation --mode quality`).
...
4. For output-producing flows (`start`, `resume`, `continue`, `feedback:`, `start change:`, `start bug:`, `import`), fix proactively before saving / showing the artifact. Do not leave a known below-quality issue in place if you can resolve it without new user input.
...
6. Ask about every missing, unclear, conflicting, keep / drop, and below-quality item that requires human input. When best practice plus the available project context can supply a defensible default for the item, the question MUST carry that recommended default with a one-line rationale so the user can accept, edit, or skip it — not a bare open question. A confirmed default is recorded as an inline assumption (`> Assumption / Validate / Change trigger`, Step 2 of the In-Generation Self-Check). A skip follows the item type's normal skip path per the `TBD` rules in § General Standards. Only normalize silently when the change is editorial or structural and does not alter the user's intended decision.
7. For `validate *` commands, report issues only and do not modify files. Conduct the run independently of the turn that generated the target — `VAL-3` for document validates, `VAL-2` for code validates (`validate implementation`, `validate bug`). Findings are graded `blocker` / `warn` / `info`. `blocker` findings prevent the corresponding `approve *` gate from succeeding. If any blocker exists, or if required structural / rule coverage evidence is missing, the validate run MUST conclude `issues-found`; it MUST NOT conclude `clean`, `pass`, or approval-ready.
...
| `SEM-1` | Semantic integrity pass | Phase artifacts, effective truth, change packs, implementation scope | blocker / warn / info | LLM-only audit compares current target against effective truth and relevant upstream / downstream contracts for contradictions, duplicated intent, terminology drift, stale assumptions, semantic cross-sprint drift, and redundant / out-of-scope content, including duplicate intent under different IDs or wording | Artifact can pass structural validation while preserving conflicting, duplicated, stale, or behavior-changing meaning |
| `CODE-1` | Code traceability marker | In-scope meaningful backend/runtime, frontend/UI, and AI/ML code surfaces | blocker for business-facing code (implements or directly serves an FR / US / BR); warn for other meaningful surfaces | Marker satisfies the canonical `CODE-1 Traceability Contract` — see `core/phase-implement.md §6` | Code surface cannot be traced to approved scope |
...
| `VAL-2` | Independent implementation verification | `validate implementation` runs (spec mode primary) and `validate bug` runs | blocker | Spec-mode audit runs independently of the generating turn per the four protocol pillars (fresh eyes; re-derived expectations; per-AC refutation; re-run, don't trust), with deterministic evidence from `core/tools/validate_implementation.py` recorded — canonical protocol: `core/phase-implement.md § Independent verification protocol`; `validate bug` applies the same pillars with the bug's repro + `VERIFIES: BUG-NNN` regression test re-run as its runtime evidence (`core/bug-manager.md § validate bug` — the plan-scoped tool does not apply) | Validate run rubber-stamps the generation turn's self-reported claims, or deterministic tool evidence is missing / contradicted |
| `VAL-3` | Independent document validation | `validate user story` / `design` / `architecture` / `plan` / `test` runs | blocker | Run re-reads the target, its source template, and upstream approved artifacts from files (fresh-context reviewer agent by default; same-context fallback only when the environment cannot open one), re-derives expectations before reading generation-turn claims, and grades each required section / rule ID by searching for its fail signal — canonical conduct: `§ General Standards → VAL-3` | Validate run's evidence only confirms section presence, restates the generating turn's claims as evidence, or the validate file does not record the expectation sources |
...
Phase-specific rules (hard / gating unless a row is marked non-gating) map to these IDs plus local IDs:
...
| `PROD-6` | Product Backlog Routing & Capture Rule | Future product ideas — **non-gating** (never blocks `approve product`); see `core/phase-product.md § Product Backlog (PROD-6)` |
...
| `TEST-3` | Rule / Branch Inventory Rule (explicit written items) | Test cases |
| `TEST-3b` | Per-AC ISTQB Technique Decision Matrix Rule | Test cases (matrix + TC title prefix) |
| `TEST-3c` | Architecture-Derived Obligations Rule (warn-only) | Test cases (inventory `[Derived]` rows) |
...
- `approve *` then re-runs the required validate command(s) as a final full confirmation pass before locking APPROVED. This approval-time re-run does not replace the explicit validate step. The re-run executes the full contract of the command it re-runs — including the `VAL-3` / `VAL-2` independence conduct (fresh-context reviewer agent by default) — it is never a lighter confirmation form.
...
- The corresponding `approve *` gate also REFUSES if the active validate file lacks `VAL-1` evidence: target fingerprint, `DOC-3` structural coverage, Rule ID coverage including `SEM-1`, semantic integrity evidence, findings, and conclusion.
- A validate file with `blocker > 0`, missing required `DOC-3` / Rule Coverage / `SEM-1` evidence, or missing phase-specific hard-rule coverage MUST record latest conclusion `issues-found`; `clean` is valid only when blocker count is zero and required coverage evidence is present.
...
- Active validate files follow `VAL-1`: they record target fingerprint, structural coverage (`DOC-3`), Rule ID coverage including `SEM-1`, semantic integrity evidence, ordered findings, counts, and conclusion.
...
Every output-producing flow MUST run this self-check loop internally **before outputting any artifact** — not just during `import` or before approval. This includes phase engines plus `feedback:`, `resume` / `continue`, `start change:`, and `start bug:` (scope-appropriate checks — a bug record runs the checks that apply to it, not the phase-artifact scans). This prevents low-quality output from reaching the user at all.
...
- Does every sequence diagram use fenced Mermaid `sequenceDiagram` syntax, not ASCII/plaintext ladder art? If not → convert it before output.
- Does the ERD relationship diagram use fenced Mermaid `erDiagram` syntax, not ASCII/plaintext entity-relationship art? If not → convert it before output.
...
- `SEM-1`: Validate commands and approval-time re-runs include a semantic integrity pass against effective truth and relevant contracts; unresolved contradictions, duplicated intent, terminology drift, stale assumptions, semantic cross-sprint drift, and redundant / out-of-scope content are findings even when IDs and structure are valid.
...
- No placeholder text such as `TBD`, `TODO`, or empty headings remains in review-ready content. Two narrow exceptions:
  - Risk-managed `TBD` is allowed ONLY for **non-functional measurable items** (NFR target, KPI baseline / target / timeframe, performance threshold, capacity number, compliance scope) — and must be captured in an open-risk, open-issues, or assumption block with owner or validator, deadline or change trigger, and downstream impact. Fixed homes: a product KPI / constraint → PRD **§10b Open Risks**; an architecture NFR target → `nfr.md` **§9 Open NFR Risks**.
  - `TBD` is **never acceptable** for structural deliverables: FR, AC, persona definition, scope boundary, testability note, traceability link, ID assignment, or any contract field (API field, ERD column, event payload field, state name, error copy). When those cannot be resolved now, do NOT invent them as `TBD`. Record them in an `## Open Questions` / `## Pending Discussion` block (what was raised, why unresolved, who needs to weigh in) and treat the affected artifact as not yet present — the corresponding next-phase gate cannot pass on it. This applies to **this-sprint structural gaps**; an Open Questions entry that explicitly targets a future phase / sprint (e.g. `→ Architecture v2`) is a deferred pointer, not a current-sprint blocker.
- Statements are specific, actionable, and decision-ready.
- IDs, terminology, and cross-links stay internally consistent.
- `VAL-1`: Active validate files must prove what was checked. They include target fingerprint, structural coverage (`DOC-3`), rule coverage by Rule ID including `SEM-1`, semantic integrity evidence, ordered findings with severity and Rule ID where applicable, and a latest conclusion.
- `VAL-2`: `validate implementation` verifies independently of the turn that generated the code. The spec-mode audit applies the four protocol pillars (fresh eyes; re-derived expectations; per-AC refutation; re-run, don't trust) and records the deterministic evidence from `core/tools/validate_implementation.py` in the active validate file. A validate run that merely restates the generation turn's self-reported claims fails `VAL-2`. Canonical protocol: `core/phase-implement.md § Independent verification protocol`. `validate bug` (`core/bug-manager.md § validate bug`) follows the same pillars — its runtime evidence is the repro + `VERIFIES: BUG-NNN` regression test re-run by the validate run itself; the plan-scoped deterministic tool does not apply.
- `VAL-3`: Document validates (`validate user story`, `validate design`, `validate architecture`, `validate plan`, `validate test`) verify independently of the turn that generated the artifact. The run: (1) re-reads the target artifact, its source template, and the upstream approved artifacts from files — spawn a fresh-context reviewer agent for this pass by default, falling back to same-context conduct only when the environment cannot open one, and follow the reviewer input contract of `core/phase-implement.md § Independent verification protocol` pillar 1 (the reviewer prompt carries only file paths and identifiers, never the generating turn's claims); conversation-memory claims from the generating turn are inputs to verify, never evidence; (2) re-derives what the artifact must contain from the template (the `DOC-3` comparison) and upstream effective truth (the `SEM-1` pass) before reading any generation-turn summary; (3) grades each required section and rule ID by actively searching for its fail signal rather than confirming presence. The active validate file records the expectation sources used (template + upstream artifact files). A validate run whose evidence only confirms presence, restates the generating turn's claims, or omits the expectation sources fails `VAL-3`. `validate implementation` follows the stricter `VAL-2` instead.
...
- `PROD-1`: Every Must Have user story satisfies the Story Readiness Rule: explicit `persona`, `AC` graded per the AC Form Contract below, `scope` (in / out), `testability`, and `traceability` to FR / NFR. This rule is the **canonical source** for the AC Form Contract; `core/phase-product.md` and `validate user story` reference it without restating; `core/templates/epic-template.md` carries the authoring-time Vietnamese rendering of the contract and MUST be updated in the same change as this section.

  **AC Form Contract**:

  - **Accepted forms** — choose per scenario; form choice by itself is never a finding:
    1. `Given / When / Then` — when the scenario has meaningful state pre-conditions.
    2. One-line `action → outcome` — when the pre-condition is obvious.
  - **Four mandatory properties** — violating any one is graded `warn` per AC, whatever the form. **Escalation predicate** (the only warn→blocker path): the story fails `PROD-1` when it has no AC that passes all four properties, or no fully-passing AC covers its happy path. Grade each AC pass / fail per property; record the per-AC result in the active validate file.
    1. **Observable** — outcome visible from outside, at least one of: named target page, exact message text in quotes, concrete UI state, visible data change. Internal-only claims ("hệ thống ghi nhận thành công") fail.
    2. **Atomic** — 1 AC = 1 scenario = one triggering action; list all observable outcomes of that action together; other branches / conditions split into separate ACs.
    3. **Testable without follow-up questions** — every quantity, message, and pre-condition the outcome depends on is stated concretely (numbers, exact copy in quotes, data conditions). Fail signal: a tester would need to ask a question before executing.
    4. **No protocol-level technical detail** (HTTP status, JSON contract, query params) — that belongs to Test Cases.
  - **Labels** — each AC carries one: `(Happy Path)`; `(Alt)` = valid alternate branch; `(Error)` = invalid input or action rejected with a visible refusal; `(Edge)` = boundary, empty state, a legitimate attempt blocked by a BR, or a time / system-triggered outcome. Missing or misleading label = `warn`.
  - **Per-US coverage guidance** (`warn`, never a blocker): ≥1 happy path; one error/validation AC per user-reachable bad input; boundary ACs when the US has limits (count / length / time); an empty-state AC when the US displays data; and ≥1 AC showing the blocked behavior for each BR the US `Respects`.
...
- An AC violates any of the AC Form Contract's four mandatory properties (see `PROD-1`).
...
- `validate architecture` MUST fail with `blocker` and latest conclusion `issues-found` when C4 lacks any of the 3 required levels (System Context, Container, Component), any required C4 Draw.io/XML source reference is missing, the package lacks at least one DFD Draw.io/XML source, a meaningful data movement is not covered by DFD, or materially different user-group flows are not split. The C4-level presence / non-empty half is deterministically enforced by `validate_proposal.py --file <architecture overview proposal> --check-c4` (rule `VP-13`; the 4th C4 level, Code, is not required), and the "≥ 1 non-empty DFD source exists" half by `validate_proposal.py --file <data-flow proposal> --check-dfd` (rule `VP-14`; missing / unlabeled arrows warn). A `VP-13` or `VP-14` blocker blocks `approve arch`.
- `VP-15`: Every New/Updated `SEQ-NNN` item contains a fenced `mermaid` code block whose diagram type is `sequenceDiagram`; ASCII/plaintext ladder diagrams are blockers and must be converted before review.
...
- `sequence` diagrams are ASCII/plaintext ladder art, prose-only, or fenced as `text` / `ascii` instead of Mermaid `sequenceDiagram`.
- `erd` relationship diagram is ASCII/plaintext entity-relationship art, prose-only, or fenced as `text` / `ascii` instead of Mermaid `erDiagram`.
...
- `TEST-3`: Rule / Branch Inventory maps every in-scope **explicit written** AC / BR / rule / branch to TC IDs or `N/A + reason`. Unmapped explicit items block `approve test`.
...
- `TEST-3c`: **Architecture-Derived Obligations** — the same inventory also includes obligations the approved architecture implies but the AC / BR never states. The single catalog of obligation types (each with its architecture trigger, what to verify, and `N/A` condition) lives in `core/standards/testing-standards.md §2.2` — do not re-list them here. Each is conditional on its architecture trigger, tagged `[Derived]`, and mapped to a TC or `N/A + reason`. **Warn-only**: missing `[Derived]` obligations surface as warnings with a recommended fix, not blockers (same policy as `TEST-3b`).
...
- `CODE-1`: New or materially changed in-scope meaningful code surfaces satisfy the canonical `CODE-1 Traceability Contract` (`core/phase-implement.md §6`); explicit pack / `Tracking IDs / ticket / task / board IDs` come from approved inputs only.
...
- In-scope meaningful code surfaces lack `CODE-1` traceability markers.
- Traceability markers are stale, generic, or inconsistent with the approved task slice.
- `Tracking IDs / ticket / task / board IDs` are invented instead of copied from approved inputs.
- Boilerplate traceability comments are sprayed across trivial helpers while the actual in-scope entrypoints remain unlinked (`CODE-2`).
