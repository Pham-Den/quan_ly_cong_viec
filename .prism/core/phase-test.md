...
6. **Rule inventory and branch coverage (`TEST-3`)**: If upstream artifacts contain AC IDs, BR IDs, rule IDs, decision tables, states, or named flow branches — **or** the approved architecture shows any Architecture-Derived Obligation trigger (the trigger column of the `core/standards/testing-standards.md §2.2` catalog) — the testing proposal MUST include a Rule / Branch Inventory (`TEST-3` explicit items + `TEST-3c` architecture-derived obligations), authored as the singleton anchored block `<!-- ID: TEST-COVERAGE-001 -->` (per `core/templates/test-cases-template.md §3`) so it is promoted into `/docs/testing/test-cases.md` at sprint seal — `## New` in sprint 1, `## Updated` (full cumulative table) later. Every in-scope item maps to concrete `TC-xxx` IDs or `N/A + reason`. Unmapped in-scope **explicit** rules (`TEST-3`) block `approve test`; missing **derived** obligations (`TEST-3c`) warn only (see the next paragraph and `core/standards/testing-standards.md §2`).

   The inventory is NOT limited to written requirement IDs. After listing the explicit items above, derive the **Architecture-Derived Obligations** from the approved architecture artifacts already loaded (API specs, sequence, ERD, data-flow, events, NFR, project / boundary reference) per `core/standards/testing-standards.md §2.2` — that section is the single catalog of obligation types, each with its architecture trigger, what to verify, and when `N/A`. For every obligation type the architecture triggers, add a row tagged `[Derived]` and map it to `TC-xxx` or `N/A + reason`. Each obligation is conditional on its architecture trigger — derive it from what the architecture actually shows, never pad. Missing `[Derived]` obligations raise warnings with a recommended fix (same policy as `TEST-3b`); they do not by themselves block `approve test`.
...
7. **Functional and SIT coverage model (`TEST-4`)**:
   ...
   - SIT coverage includes API contract/schema, cross-service flow, retry/idempotency, rollback or compensation, async event / webhook / queue paths, cross-system consistency, and backward-compatibility / breaking-change detection when contracts or consumers are versioned.
...
Test artifacts must satisfy `DOC-1`, `DOC-2`, `DOC-3`, `LINK-1`, `LINK-2`, `ORB-1`, `TEST-1`, `TEST-2`, `TEST-3`, `TEST-3b`, `TEST-3c`, `TEST-4`, `TEST-5`, `TEST-6`, `TEST-7`, and `TEST-8`.
...
> **Reflect sprint (`kind: reflect` — `core/sprint-manager.md § Reflect Sprint`):** a reflect sprint has **no Test lane** — `approve test` is **not required** and the `TEST-*` gates below do not apply. The reflect sprint seals via `approve implement` without test APPROVED (see `core/phase-implement.md § Gate` reflect exception). Untested reflected behavior is recorded as a known gap, not blocked.
...
- `VAL-1`: the active validate file records structural coverage and rule coverage for `DOC-1`, `DOC-2`, `DOC-3`, `LINK-1`, `LINK-2`, `ORB-1`, `SEM-1`, `TEST-1`, `TEST-2`, `TEST-3`, `TEST-3b`, `TEST-3c`, `TEST-4`, `TEST-5`, `TEST-6`, `TEST-7`, and `TEST-8`.
- `SEM-1`: run the semantic integrity pass across Test, approved Product / Design / Architecture, and effective truth: catch test cases that contradict AC / BR / API / state / NFR semantics, duplicated coverage under different TC IDs, terminology drift from glossary / API names, stale assumptions after upstream changes, redundant cases, and out-of-scope execution expectations.
...
- `TEST-3c`: Architecture-Derived Obligations — for each obligation type in the `core/standards/testing-standards.md §2.2` catalog that the approved architecture triggers, the inventory has a `[Derived]` row mapped to a TC or `N/A + reason`. **Warn-only** — missing `[Derived]` obligations are reported with a recommended fix and do not block `approve test`.
