...
5. `product backlog: N active` — only when N > 0 (count only; never the item list)
6. `open bugs:` — DRAFT/ACTIVE bug records, only when at least one exists (non-gating; never a seal / `new sprint` blocker)
7. `next: ...`
...
Bug records are sprint-independent and non-gating (`core/bug-manager.md § Non-Gating Rule`). List them for visibility only — they never appear as a sprint-seal or `new sprint` blocker. Show this section only when at least one DRAFT/ACTIVE bug exists, after the sprint blocks and before `next:`:

```text
open bugs:
  BUG-007-login-401  [ACTIVE]  severity: urgent  doc-impact: none
  BUG-009-csv-export [DRAFT]   severity: normal  doc-impact: suspected
```

List `severity: urgent` first. Each line shows `BUG-NNN-{slug}`, status, severity, and doc-impact.
