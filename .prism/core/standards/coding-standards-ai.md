...
AI / ML code MUST apply the canonical `CODE-1 Traceability Contract` from `core/phase-implement.md §6`. Use the placement example below for AI / ML surfaces such as inference handlers, agent capabilities / tools, training or evaluation steps, evaluators, guardrails, and model-dependent orchestration functions.
...
# Model: model-card §RetrySuggest-v1.3 | Pack: v2.3.8-fix-payment | Tracking: PAY-114
def suggest_retry(...):
    ...
```
...
- Omit `Tracking:` when no approved `Tracking IDs / ticket / task / board IDs` exist. Never invent one.
- Include `Model:` whenever the function depends on a specific model version — the model card is that surface's contract ref. Link to the model card.
- `CODE-1` overrides local no-comment legacy style; nearby old code without markers is not a valid pattern to copy.
...
6. Does every in-scope AI / ML surface satisfy the canonical `CODE-1 Traceability Contract`, including `Model:` when applicable?
