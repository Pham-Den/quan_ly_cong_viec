...
Frontend code MUST apply the canonical `CODE-1 Traceability Contract` from `core/phase-implement.md §6`. Use the placement example below for frontend / UI surfaces such as pages, routes, business-facing components, hooks, store slices, API services, and migration / build-time artifacts.
...
// Project: /docs/architecture/project-reference.md §PR-005
...
6. Does every in-scope frontend / UI surface satisfy the canonical `CODE-1 Traceability Contract`?
