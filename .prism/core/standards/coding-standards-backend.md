...
Backend code MUST apply the canonical `CODE-1 Traceability Contract` from `core/phase-implement.md §6`. Use the placement example below for backend / runtime surfaces such as API handlers, controllers, application / domain services, jobs, migrations, event handlers, and non-trivial business functions.
...
6. Does every in-scope backend / runtime surface satisfy the canonical `CODE-1 Traceability Contract`?
