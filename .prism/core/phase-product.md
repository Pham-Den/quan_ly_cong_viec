...
2. **Question categories** (ask comprehensively):
   ...
   - **Entity lifecycle, states & actor permissions**: Which business entities have meaningful lifecycle states (order, subscription, refund, approval item, invoice, support ticket, etc.)? What states exist, what triggers each transition, which transitions are invalid, and are there timeout / expiry / cancel / retry / escalation rules? Across the product's roles and protected surfaces — lifecycle-entity transitions AND any other gated action or view (admin screens, reports, exports, configuration): which roles may perform which actions and see which data, and does any action require another role's approval? Confirmed permission rules become access-rule `BR` items in the PRD (§6.3).
     - *Depth probe*: "List 1–3 key entities. For each entity: current states, transition trigger, invalid transitions, any timeout / expiry rule — and which role is allowed to trigger each transition or view the entity's data."
     - *Reject vague*: "Đơn hàng có nhiều trạng thái" → ask: "Cụ thể là gì? Ví dụ `PENDING -> CONFIRMED -> SHIPPED -> DELIVERED`; transition nào bị cấm, và timeout / cancel sau bao lâu?" / "Admin quản lý được hết" → ask: "Liệt kê theo cặp role × action: role nào được tạo / duyệt / hủy / xem gì? Hành động nào cần phê duyệt của role khác?"
   ...
   - **Integrations & legacy migration**: Hệ thống nào cần tích hợp? Hướng dữ liệu (đẩy ra / nhận vào / hai chiều)? Realtime hay batch? Owner của hệ thống bên kia? Có SLA cam kết? Có hệ thống cũ bị thay thế không — dữ liệu / người dùng hiện hữu có cần migrate một lần không (khác với tích hợp ongoing), và hai hệ thống có chạy song song trong giai đoạn chuyển tiếp không? *(Chi tiết kĩ thuật — protocol cụ thể, field-level mapping, mã lỗi, retry — thuộc phase Architecture; mã / dữ liệu test thuộc phase Test.)*
     - *Depth probe*: "For each integration: (1) Tên hệ thống? (2) Hướng dữ liệu? (3) Realtime hay batch? (4) Owner & SLA? — và nếu có hệ thống cũ: (5) Dữ liệu / user nào cần migrate — bao nhiêu bản ghi, chất lượng nguồn ra sao — một lần hay chạy song song?"
     - *Reject vague*: "Tích hợp với hệ thống HR hiện tại" → ask: "Hệ thống HR cụ thể là gì? Ai sở hữu? Hướng dữ liệu nào? Realtime hay batch?" / "Chuyển từ hệ thống cũ sang" → ask: "Dữ liệu nào phải mang sang (bao nhiêu bản ghi, chất lượng ra sao)? Cutover một lần hay chạy song song bao lâu?"
...
3. **Probe vague answers — Suggest, don't just reject** (Product-phase application of the ask-with-recommended-default rule — canonical: `core/phase-quality-standards.md § How To Apply`): If a user answer is vague (no numbers for NFR/KPI, no specific feature action, no concrete constraint), do NOT simply reject. Instead:
   - Re-ask with a specific probe AND offer concrete defaults/examples as suggestions, per the canonical rule.
   ...
   - **User confirms a default**: Accept it and record it per the canonical rule — an inline `> Assumption: [...] / Validate: [...] / Change trigger: [...]` block on the affected item, not a free-form note.
...
8. **Story readiness rule (`PROD-1`, hard rule)**: Every Must Have user story in product split proposals under `product/proposals/` (the active sprint's slice) plus carried-over Must stories from `/docs/product/epics/EP-NNN-{slug}.md` MUST have:
   ...
   - acceptance criteria (`AC`) present and graded per the AC Form Contract (canonical: `core/phase-quality-standards.md § Product Standard → PROD-1`) — per-AC violations are `warn`; the story fails `PROD-1` only per the contract's escalation predicate
...
12. **Product Backlog routing & capture (`PROD-6`)**: When a future / out-of-scope / "maybe later" product idea surfaces — during interrogation, while writing, or at any other point — do not silently expand it into the current package. Route it per `§ Product Backlog (PROD-6)`: in-sprint homes first for decided-later work, the Backlog only for an unscheduled / speculative / unrelated product idea (and only through the capture trigger defined there). `PROD-6` is non-gating — it never blocks `approve product`.
...
- Optional — `docs/product-backlog.md` `## Active` items: read to surface relevant future ideas at `start product` (and as seed context when one is carried). See `§ Product Backlog (PROD-6)`. Not a package artifact; never merges to Living Truth on its own
...
> **Reflect sprint (`kind: reflect` — `core/sprint-manager.md § Reflect Sprint`):** the **first two** bullets below — `PROD-1` Must-Have story readiness, and Must-FR→Must-US mapping — are **exempt** (reflected items describe running code, not specified user stories; a reflected FR may exist without a Must-Have story). All other bullets STILL apply: `PROD-4` traceability map, `PROD-2`/`PROD-3` where relevant, the active validate file — and the `validate user story` audit that writes it applies these **same** exemptions, so it must not re-flag a reflected FR that has no Must-Have story (otherwise bullet "active validate file clean" would silently re-impose `PROD-1`) — and `validate_proposal.py` structural checks.
> **KPI / NFR / constraint vague bullet (below) in reflect:** NOT exempt, but **satisfied — not fabricated**. As-built has no real KPI/NFR (code holds no WHY); at `import` every KPI/NFR/constraint that cannot be quantified from code is routed to the PRD **§10b Open Risks** as a deferred open risk per `PROD-2` (and broader intent to a §11 deferred Open Question) — see `core/import-validator.md § Reflect Mode → Deferred intent`. The `validate user story` audit must therefore treat an unquantifiable-from-code KPI/NFR as a **deferred §10b open risk (non-blocking)**, never hard-block demanding a fabricated target; it still flags a reflected KPI/NFR that is vague **and** has no §10b row.
> **DOC-3 intent-only PRD sections in reflect:** the WHY-bearing sections code cannot supply — §2 Problem Statement, §3 Goals & Success Metrics (its KPI / success-metric targets route to §10b, its business/user-goal narratives to §11), §4 TO-BE flow, market research, and persona *rationale / goals* (the code/role-derivable actor identity / `PERSONA-NNN` block itself still holds — only its WHY defers; and reflect has no Must-Have US forcing a persona reference, so `phase-quality-standards.md`'s "persona definition is not TBD-able" structural rule is not in tension here) — are satisfied by `N/A — reflected, intent deferred (Skip Register / §11 deferred Open Question)`, never left blank (blank = `DOC-3` blocker) and never fabricated. The AS-IS half of §4 and any code-derivable BR / lifecycle (`PROD-3`) STILL hold. Forward (non-reflect) sprints keep these sections mandatory and ASK the user — see `core/sprint-manager.md § Reflect Sprint` rule 8 "WHY-gap deferral".

- a Must Have user story violates `PROD-1` (missing `persona`, `AC`, `scope`, `testability`, or `traceability` to FR / NFR — or its AC set fails the AC Form Contract's escalation predicate)
...
2. `AC` section is present; grade every AC against the AC Form Contract (canonical: `core/phase-quality-standards.md § Product Standard → PROD-1`) by the four mandatory properties — never by form choice (GWT vs one-line). Per-AC violations are `warn`; blocker only per the contract's escalation predicate. Record the per-AC result in the validate file
...
- `VAL-1`: the active validate file records structural coverage and rule coverage for `DOC-1`, `DOC-2`, `DOC-3`, `LINK-1`, `LINK-2`, `ORB-1`, `SEM-1`, `PROD-1`, `PROD-2`, `PROD-3`, and `PROD-4`.
- `SEM-1`: run the semantic integrity pass across product effective truth and the current product proposals: catch contradictory business rules / KPIs / NFRs / personas / market assumptions, duplicated intent under different EP / FR / US / BR IDs, terminology drift from glossary, stale assumptions from previous sealed truth or approved change packs, and redundant or out-of-scope requirements.
...
⚠ warn [PROD-1]: US-019 AC-041 outcome not observable — "hệ thống ghi nhận thành công" has no visible signal / exact copy (AC Form Contract property 1)
...
## Product Backlog (PROD-6)

The Product Backlog captures future product ideas that are not yet any sprint's scope, so they are neither lost in chat nor forced prematurely into the current package. It is product-owned and **non-gating**: it never blocks `approve product`, and design / architecture / plan / implement never load it.

**File** — `docs/product-backlog.md`: a single flat file with `## Active` and `## Archive` sections, created lazily on first use. Items use loose internal `#n` numbering (AI-managed, NOT real `EP/FR/US/AC` IDs; dropped on carry). **Minimal Active-item shape**: a short title with its `#n`, a `target` hint (a sprint like `v3`, or `post-MVP` / `unscheduled`), and a one-line note — optional `area`; just enough to surface by target and decide later, never a PRD / epic / AC. The section an item sits in IS its status; an archived item keeps a one-line reason plus, when carried, a sprint backlink. It lives outside `/docs/product/` and sprint folders, so `seal_sprint.py` never touches it. Identical in guided and freedom mode; no DRAFT / approval status.

**Routing first — Backlog last.** Most "later" content already has a home; route by certainty × architectural coupling and only fall through to the Backlog for an unscheduled / speculative / unrelated product idea:

| The "later" item | Home |
|---|---|
| Decided next phase that constrains this sprint's architecture/design | PRD §5 Out-of-Scope `[scheduled vN]` + §4.2 Future State + an ADR seam |
| Decided next phase, independent of the current sprint | PRD §5 Out-of-Scope `[scheduled vN]` |
| Decided to do, detail still open | PRD §11 Open Questions row naming the target phase |
| Technical / non-functional future | NFR + ADR (never the Backlog) |
| Rejected scope | PRD §5 Out-of-Scope `[rejected]` + reason |
| Unscheduled / speculative / unrelated product idea | the Backlog |

In-sprint homes seal into Living Truth, so the next sprint inherits them automatically (see `core/sprint-manager.md`). Never leave a half-specified item inside the current package: commit it complete (real IDs, full AC) or record it as a one-line pointer — never a stub.

**Capture trigger — the file is NOT a conversation log.** Write to the Backlog only when **(a)** the user explicitly asks to save it, or **(b)** you are confident it is a persist intent (echo the one-line entry as you write). **(c)** You may proactively *offer* to save a strong, reusable idea the user did not flag, but write only on a *yes*. Whenever it is unclear, **ask — do not write**; keep offers rare and never nag. The signal is the user's act of asking to remember, not the future tense of the topic.

**Surfacing.** At `start product` — and at `new sprint` — surface relevant `## Active` items by target hint (exact target first; loose hints like `post-MVP` / `unscheduled` shown but not pushed) and ask whether to carry any. Never auto-carry. Guided `new sprint` additionally surfaces sealed-in deferrals — see `core/sprint-manager.md`.

**Carry into a sprint** (no bespoke pipeline): diff the `#n` note against the target sprint's product effective truth (`python .prism/core/tools/effective_truth.py --phase product --up-to-sprint v{N}`, where `v{N}` is the sprint being carried into — bound to it so a later open sprint's proposals are not pulled in) and warn on overlap, then seed `start product` (or `import product` if a full external doc now exists). Move the `#n` item to `## Archive` with a sprint backlink. Never auto-carry; always ask.

**Boundary.** The Backlog holds uncommitted ideas only. Modifying already-committed scope is never a Backlog action — an APPROVED current-sprint artifact uses `start change:`; a sealed sprint routes per `core/sprint-manager.md § Cross-Sprint Routing` (newest open sprint, or `new sprint` if none is open).
