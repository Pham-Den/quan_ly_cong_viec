...
- Every diagram MUST be inside a fenced `mermaid` code block whose diagram type is `sequenceDiagram`
- ASCII art, plaintext ladder diagrams, `text` / `ascii` code fences, or prose-only flow descriptions are not valid sequence diagrams
...
- [ ] Không có ASCII/plaintext ladder diagram thay cho Mermaid `sequenceDiagram`
