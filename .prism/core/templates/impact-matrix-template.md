...
<!-- Machine-checked by `python .prism/core/tools/seal_sprint.py --sprint v{X} --check-pack-structure --pack {pack-id}`:
     • Keep all six phase rows; `Impacted?` must start with `yes` or `no`.
     • For Product / Design / Architecture / Test rows: write the word `delta` in `Action now`
       exactly when this pack creates a delta file for that phase (e.g. `delta: product-delta-….md`);
       the tool cross-checks the row against the pack's `{phase}-delta-*.md` files BOTH ways
       (a `delta` row with no file, and a delta file with a non-impacted row, are blockers).
     • The machine check covers only those four seal-merged delta types. Plan absorption
       (`feedback:` on a DRAFT plan, or a Plan delta per `core/phase-plan.md` when APPROVED — a
       `plan-delta-*.md` never merges to Living Truth) and Implement absorption (the current
       implementation pass) are judged by the validate pass, not this tool
       (`core/change-propagation.md § Per-Phase Handling`). -->
