...
## 9. Open NFR Risks (target TBD / deferred)

<!-- "Nhà" cố định cho mọi NFR target / control chưa đo được hoặc chưa chốt trong phase này — đối xứng với PRD §10b ở phía product. -->
<!-- Không để TBD/vague im lặng trong §1–§8: kéo vào đây với owner, downstream impact, default (nếu có), và deadline rõ ràng. -->
<!-- `validate architecture` / `approve arch` flag một NFR target vague/TBD KHI KHÔNG có dòng tương ứng ở đây (risk-managed-`TBD`, `prism/core/phase-quality-standards.md`). Có dòng ở đây = đã quản trị, không còn là "NFR unmeasurable" blocker. -->
<!-- Reflect / as-built (`kind: reflect`): NFR mà code không cho biết target (vd: thấy timeout=30s nhưng không có SLA) → park ở đây dưới dạng declared-deferred (owner = PO, deadline = "lần PO chạm area này"), KHÔNG bịa số. Forward sprint (greenfield hoặc thêm yêu cầu) thì phải HỎI user — xem `core/sprint-manager.md § Reflect Sprint` rule 8 "WHY-gap deferral". -->
<!-- OPTIONAL — section này chỉ cần present khi có NFR target bị hoãn. Để trống / "None" / vắng hẳn section = ĐÃ thỏa, KHÔNG phải DOC-3 blocker; một `nfr.md` cũ (seal trước khi có §9) thiếu §9 KHÔNG bị retro-flag — đối xứng cách PRD §10b xử lý. -->

| # | NFR (ID nếu có) | Cái gì còn vague / TBD | Tại sao quan trọng (Architecture / Test / Implement impact) | Default tạm dùng (nếu có) | Ai validate | Hạn chót | Trạng thái |
|---|---|---|---|---|---|---|---|
| 1 | <!-- VD: NFR-007 --> | <!-- VD: p95 latency target chưa chốt --> | <!-- VD: chưa chốt được caching strategy nếu thiếu số --> | <!-- VD: 500ms — baseline web SaaS --> | <!-- VD: PO + Tech Lead --> | <!-- VD: trước approve plan --> | Mở |
| 2 | | | | | | | Mở |
...
- [ ] Mọi NFR đều có target đo được — hoặc target chưa chốt được đã park ở §9 Open NFR Risks (không để TBD im lặng)
...
- [ ] Mọi NFR target còn vague / TBD đã có dòng ở §9 Open NFR Risks (owner, downstream impact, default nếu có, deadline)
