...
<!-- Required when upstream artifacts contain AC IDs, BR IDs, Rule IDs, decision-table rows, named flow branches, or state transitions — OR when the approved architecture shows any Architecture-Derived Obligation trigger (the trigger column of the `core/standards/testing-standards.md §2.2` catalog). Few/no explicit IDs is NOT an exemption if the architecture shows obligation triggers. -->
<!-- Every in-scope item must map to TC IDs or have N/A + reason. -->
<!-- The inventory has TWO sources (see `core/standards/testing-standards.md §2`):
     (1) Explicit written items (AC / BR / rule / branch / state — blocking when unmapped), and
     (2) Architecture-Derived Obligations tagged `[Derived]` — behaviors implied by the approved
     architecture. The single catalog of obligation types lives ONLY in `core/standards/testing-standards.md §2.2`.
     Add ONE `[Derived]` row per applicable obligation type from that catalog; the two rows below are
     illustrative (a generic placeholder + a worked AuthZ example), NOT the full list — read §2.2 for it.
     Map each to TC or `N/A + reason`. Missing `[Derived]` rows warn, not block. -->
...
<!-- Examples below are ILLUSTRATIVE (row shape only) — the authoritative obligation-type list is `testing-standards.md §2.2`. -->
| `[Derived]` {ObligationType}: {target} | `/docs/architecture/{source}.md §...` | what to verify — per this obligation's rule in `testing-standards.md §2.2` | TC-NNN | covered / N/A | N/A reason per §2.2 |
| `[Derived]` AuthZ-boundary: {operation} | `/docs/architecture/api-specs.md §API-NNN` | who allowed/blocked; each enforcement layer + bypass path *if architecture shows them* | TC-NNN | covered / N/A | N/A reason if single-layer / no access control |
...
|---|---|---|---|---|---|---|---|---|---|
| | TC-NNN | TC-NNN | TC-NNN | TC-NNN | TC-NNN | TC-NNN | TC-NNN | TC-NNN / N/A (unversioned) | |
...
- [ ] Quality Contract refs satisfied: `DOC-1`, `DOC-2`, `DOC-3`, `LINK-1`, `LINK-2`, `ORB-1`, `TEST-1`, `TEST-2`, `TEST-3`, `TEST-3b`, `TEST-3c`, `TEST-4`, `TEST-5`, `TEST-6`, `TEST-7`, `TEST-8`
...
- [ ] **`TEST-3c` Architecture-Derived Obligations**: for every obligation type in the `core/standards/testing-standards.md §2.2` catalog that the approved architecture triggers, the inventory §3 has a `[Derived]` row mapped to a TC or `N/A + reason` (warn-only)
