...
<!-- Phase 9+ production proposal artifact. `apply_proposal.py` merges this file into the
     ...
     • Reflect sprint (`kind: reflect` — brownfield reflected, see core/sprint-manager.md
       § Reflect Sprint): mark provenance INSIDE the block body, e.g. a line
       `**Provenance**: reflected — <path:line> (confidence: verified|inferred)`.
       Keep it in the body (copied verbatim into Living Truth at seal); do NOT put it in
       file frontmatter (seal restamps frontmatter to a fixed schema and would drop it).
-->
...
ID prefix → Living Truth file routing
  ...
  SEQ-NNN                      | /docs/architecture/sequence.md                           | Sequence diagram (fenced Mermaid `sequenceDiagram` only)
...
<!-- EP body matching epic-template.md root sections, ĐỦ: Epic Overview (bảng) + Bối cảnh + Vấn đề cần giải quyết + Giá trị mang lại + Tiêu chí nghiệm thu cấp Epic + Phụ thuộc & Ghi chú + required `Product Traceability Map` table (`EP -> FR -> related US`) + Edge cases & câu hỏi mở (nếu có). -->

<!-- Example: a new FR (use the owning product/proposals/epics/EP-XXX-{slug}-v{X}.md) -->
<!-- ID: FR-NNN -->
<!-- EPIC: EP-XXX -->
**FR-NNN — {{Capability name}}**
<!-- FR body matching epic-template.md FR block: mô tả + Phạm vi + Trace (Covered by US / Verifies KPI) + Ghi chú (final state). -->

<!-- Example: a new user story (use the owning product/proposals/epics/EP-XXX-{slug}-v{X}.md) -->
<!-- ID: US-NNN -->
<!-- EPIC: EP-XXX -->
### US-NNN: {{Story title}}
<!-- Story body matching epic-template.md US block, ĐÚNG THỨ TỰ: (1) narrative blockquote BẮT BUỘC "Với tư cách là … / Tôi muốn … / Để …" → (2) bảng thuộc tính → (3) AC list → (4) bảng Assumption / Validate / Change trigger. -->
...
<!-- ID: AC-NNN -->
<!-- US: US-XXX -->
**AC-NNN ({{Happy Path|Alt|Error|Edge}})**
...
<!-- FR body matching epic-template.md FR block: mô tả + Phạm vi + Trace + Ghi chú (NEW final post-merge state). -->
