...
<!-- BẮT BUỘC: Diagram quan hệ phải nằm trong fenced `mermaid` block có diagram type là `erDiagram`.
     KHÔNG dùng ASCII/plaintext ER art, `text`/`ascii` fence, hay mô tả prose thay cho diagram. -->
...
- [ ] §2 Relationship Summary dùng fenced Mermaid `erDiagram` — không ASCII/plaintext ER art, `text`/`ascii` fence, hay prose
