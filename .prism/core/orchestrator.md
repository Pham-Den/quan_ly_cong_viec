...
| "fix it", "it's broken / crashing / wrong" (behavior contrary to what should already hold) | `start bug:` — confirm it is a defect, not a new feature; if no repro / expected / actual yet, capture them into the bug record before editing code |
| "implement X", "build / add this feature", "write the code" | requirement door: existing Implement routing (`start implement` / `feedback: implement`); if no approved plan / valid implement lane → `ORIGIN-1` STOP + ask |
...
| `validate implementation` | Aggregate convenience form of the two required Implement audits (same pattern as `validate changes`): runs `--mode spec` first, then `--mode quality`, updating both active validate files in one pass. Not a third audit surface; see `core/phase-implement.md § Validate Implementation Command` |
| `start bug: [symptom]` | Open a sprint-independent bug record (`BUG-NNN` under `docs/bugs/`) for a reproducible defect in accepted/sealed code — the authorized origin for the code fix. See `core/bug-manager.md` |
| `validate bug [BUG-NNN\|slug]` | Read-only audit of a DRAFT/ACTIVE bug: reproduction demonstrated (or substantively justified), regression test `VERIFIES: BUG-NNN` passes, fix in declared scope, doc-impact assessed — required before `approve bug` |
| `approve bug [BUG-NNN\|slug]` | Close a bug (`RESOLVED`) after `validate bug` is clean + fresh, the regression test passes, the fix is in scope, and doc-impact is resolved. Does not seal anything |
...
## Traceable-Origin Gate (ORIGIN-1)

Every code mutation in Guided mode must trace to a tracked origin. This is the single choke point for "may I edit this?" — phase engines do NOT duplicate code-edit blocks; they enforce in-scope quality / boundary (`CODE-5`, Scope Discipline) only AFTER this gate grants the lane.

**Scope — by effect, not by filename.** ORIGIN-1 applies before the FIRST create/edit/delete in a turn of anything that **affects software behavior or build** (source code, repo tests, build / dependency / config files such as `Dockerfile`, `package.json`, CI, ...) OR is a **governed PRISM artifact** (product / design / architecture / plan / test-plan / test-cases proposals, change packs, bug records). It does NOT apply to purely informational files that do not change behavior (`README`, `LICENSE`, `CHANGELOG`, plain docs, comments, ...). Read-only inspect / `status` / `validate *` are not mutations.

**Step 1 — Detect state** (cheap; cache-aware). Read `prism-config.md` (sprints + `sealed`), the current sprint's phase / lane statuses, DRAFT change packs, and DRAFT/ACTIVE bug records. Determine: is an implement pass open (plan APPROVED, sprint not sealed)? is an implementation lane active? any open / approved change pack to absorb? any open bug? is the latest sprint sealed? If the session already established the lane and nothing changed (no new approve / seal / config edit), reuse it — do not re-read (same cache-awareness as the adapter's Phase Engine Loading).

**Step 2 — Classify the mutation into one KIND and route. This table is the whole specific logic — do not repeat it elsewhere:**

| Kind | Mutation touches | State | Route |
|---|---|---|---|
| DOC | governed PRISM artifact | not yet created | `start [phase]` / `import` / `start change:` / `start bug:` per artifact type |
| DOC | ↑ | DRAFT | `feedback: [phase]`; `import [phase]` when new source material is in `docs/inbox/` (folds into the draft) |
| DOC | ↑ | APPROVED, sprint open | `start change:`; `import [phase]` with new inbox material (folds into a change pack — `core/import-validator.md § Import Into A Change Pack`) |
| DOC | ↑ | sprint sealed | newest open sprint (`start change:`), else `new sprint` |
| DOC | looks like a PRISM / project spec but phase / ownership unresolved | unknown / no active phase | STOP + classify first (Product / Design / Architecture / Plan / Test / Change / Bug / user-owned note); if NOT a PRISM artifact, edit per clear intent; otherwise route by the matching row |
| CODE | source code / repo test | implement pass open (approved plan OR absorbed approved change in the implement lane; sprint not sealed) and the request resolves via existing Implement routing | OK into Implement routing (`start implement` / `feedback: implement`); ORIGIN-1 adds no new task-slice requirement |
| CODE | ↑ | a DRAFT/ACTIVE bug record with declared scope exists (repro + expected / actual + `allowed_diff_boundary`) | OK (`feedback bug:` / continue the `start bug:` lane); whether the edit stays inside that scope is caught by `validate bug` / Scope Discipline afterwards, not by ORIGIN-1 |
| CODE | ↑ | no origin | STOP + ask (message below) |

**Step 3 — No valid origin → STOP.** Do not mutate silently. Print ≤3 lines of detected state + the two doors and ask the user to choose:

```text
⚠ Need a tracked origin before editing <code|test|doc>.
State: sprint-v{X} · plan: {APPROVED|—} · pass: {open|closed} · sealed: {yes|no} · bug: {BUG-NNN|—}
Pick a door:
→ New requirement / spec change : start [phase] | start change: … | new sprint | start implement: <task-slice>
→ Defect (reproducible)         : start bug: <symptom>
```

ORIGIN-1 keeps the existing Implement routing — it does not force the user to retype `start implement` or name a task slice each turn.

**Step 4 — Ambiguous** (DOC vs CODE unclear; an unresolved-ownership doc; new-behavior vs bug; or several active bug / change / implement lanes plausible) → default to STOP + ask which target. Never guess into a silent edit.

**Step 5 — Single choke point.** No other file duplicates code-edit blocking; every phase engine assumes ORIGIN-1 already authorized the lane.

## Same-Sprint Change Routing

`start change:` is not a normal document phase. It opens a same-sprint change pack managed by `core/change-manager.md` and propagated by `core/change-propagation.md`. **"Same-sprint" names the pack's *mechanism* — its delta stays within the one sprint it is opened in; *which* sprint that is gets resolved per `core/sprint-manager.md § Cross-Sprint Routing` (rule 1 below), so a pack may target the newest open sprint or an explicitly named unsealed one — it is not restricted to the current sprint.**

Rules:

1. Resolve the target sprint first: **default the newest open sprint**, or an explicitly named **unsealed** sprint (per `core/sprint-manager.md § Cross-Sprint Routing`; being *older* is not a refusal). Then use `start change:` only when the source is `APPROVED` in that sprint; if it is still `DRAFT` there, route to `feedback:` instead (next rule).
...
6. Do not guess.
7. This resolves *which artifact/lane*. Which open **sprint** the target lives in (older / sealed / shared-anchor handling) follows `core/sprint-manager.md § Cross-Sprint Routing`.
...
| test (`approve`) | product + design + architecture APPROVED + `TEST-1`, `TEST-2`, `TEST-3`, `TEST-3b`, `TEST-3c`, `TEST-4`, `TEST-5`, `TEST-6`, `TEST-7`, and `TEST-8` satisfied, including `TEST-2` execution-ready + implementation-consumable handoff, `TEST-3b` trace + Technique title-prefix discipline (and `TEST-3c` architecture-derived obligations — both warn-only), and `TEST-8` generated TSV exports | active `validate test` file is present, fresh, clean, approval-time re-run finds no new gaps, and `python .prism/core/tools/validate_proposal.py --file docs/sprint-v{X}/testing/proposals/test-cases-v{X}.md` returns 0 blockers | All previous sprints must be `sealed` |
...
0. **Reflect-sprint short-circuit:** if the target sprint's `prism-config.md` entry has `kind: reflect`, follow `core/sprint-manager.md § Reflect Sprint` instead of the build/spec gates below — that section + each `phase-*.md` Gate define exactly which build/spec checks are waived. Product / architecture / design approvals and ALL structural seal validators (`validate_proposal.py`, `validate_living_truth.py`) STILL apply. The steps below are for normal (non-reflect) sprints.
...
Aggregate commands own no slug of their own: `validate changes` writes the impacted phases' normal files, and the aggregate `validate implementation` (no `--mode`) writes/updates both `implementation-spec` and `implementation-quality` files — never a combined file.
...
- expectation sources (document validates, `VAL-3`): the source template + upstream artifact files the run re-derived its expectations from
- rule coverage by Rule ID, including phase-specific rules and shared rules such as `DOC-*`, `LINK-*`, `ORB-*`, `SEM-1`, `CODE-*`
- semantic integrity evidence (`SEM-1`): contradictions, duplicated intent, terminology drift, stale assumptions, semantic cross-sprint drift, and redundant / out-of-scope content checked against effective truth and relevant contracts
...
- `clean` is allowed only when `blocker: 0` and every required structural / rule / semantic integrity check for the validate command is explicitly present.
- `issues-found` is mandatory when any `blocker` exists, when `DOC-3` / Rule Coverage / `SEM-1` evidence is missing, or when a phase-specific hard rule is missing required coverage.
...
Use `Finding: none` only after checking the current target against effective truth and relevant upstream / downstream contracts for contradictions, duplicated intent under different IDs, terminology drift, stale assumptions, semantic cross-sprint drift, and redundant / out-of-scope content.
...
Missing `DOC-3` structural coverage, Rule Coverage, or `SEM-1` semantic integrity evidence in an active validate file is itself a `blocker [VAL-1]` for approval because the approval gate cannot prove which output sections and rules were audited.
...
When the user types `approve [phase]`, the gate first checks the required active validate file(s). If they are present, clean, and not stale, `approve` then re-runs the required validate command(s) in console-only mode as a final full confirmation pass. The re-run executes the full command contract, including the `VAL-3` / `VAL-2` independence conduct (see `core/phase-quality-standards.md § Validate Commands And Approve Gates`) — console-only refers to where findings go, not to a lighter audit.
...
1. The newest open (unsealed) sprint is the default for new work; for `resume`/continuation, consider **all open sprints** — not only the highest — per `core/sprint-manager.md § Cross-Sprint Routing`
2. Scan all documents across the open sprints for their `status` and `updated` frontmatter
...
12. Scan `docs/bugs/BUG-NNN-{slug}/` for DRAFT/ACTIVE bug records. Bugs are sprint-independent and non-gating: list them for visibility and `resume` targeting, but they NEVER block sprint seal or `new sprint` (`core/bug-manager.md § Non-Gating Rule`). An active bug lane is a plausible `feedback:` / `resume` target like any other active target.
...
6. List all DRAFT/ACTIVE bug records (`docs/bugs/`) per `core/status-format.md § Open Bugs Format` — id, slug, status, severity, doc-impact; highlight `severity: urgent`. Bugs are non-gating and must never be rendered as a seal / `new sprint` blocker.
...
If `review implement` is ambiguous between spec correctness and code quality, prefer `validate implementation --mode spec` first and tell the user to run `validate implementation --mode quality` after spec is clean. Do not run both silently unless the user explicitly asks for both — typing `validate implementation` with no `--mode` IS the explicit ask for both: run the aggregate form (spec first, then quality, one pass).

### Change Pack Aggregate

`validate changes [pack-id|slug]` is a read-only aggregate command for DRAFT change packs. It does not create `validate-changes-*.md`. It resolves exactly one change pack, runs the deterministic pack-structure check, re-derives the impacted phases independently before reading the pack's `impact-matrix.md`, and then runs the existing validate commands for that pack cycle — full conduct: `core/change-manager.md § validate changes` and `§ Pack Structure Check`.
...
For `validate changes`, repeat this process for each underlying phase command identified above, then output an aggregate result ordered by severity and phase order. Also run `core/change-manager.md § Same-Sprint Collision Check` (scoped to this pack) and fold any reported same-sprint cross-source collision into the aggregate findings. The pack is clean only when every required underlying validate result is clean and fresh and no unresolved collision involves the pack.
...
- **Persisted result**: every validate command updates exactly one active validate file. Aggregate forms (`validate changes`, `validate implementation` with no `--mode`) own no file of their own — they update the files of the underlying commands they run (see `§ Validate Active Files § Naming And Cycle`).
...
7. Run `core/change-manager.md § Same-Sprint Collision Check` scoped to this pack. If it reports a same-sprint cross-source collision involving the pack (the same anchor changed by 2+ sources), do not silently close — surface it and have the user reconcile (refresh the pack to subsume the other source) or explicitly confirm the version-order outcome. This is the early twin of the seal-time guard, which hard-blocks the same condition regardless.
...
5. If the resolved target is the current implementation lane, apply feedback only to the active code-execution scope for this session and preserve or update `CODE-1` feature-linkage markers on touched in-scope meaningful code surfaces
...
If the user tries to modify an APPROVED document via `feedback:` or a direct edit request in an open sprint, block with a clear refusal and the next valid step (`import [phase]` is routed, not refused: its payload is inbox files that `start change:` cannot carry — it folds into a change pack per `core/import-validator.md § Import Into A Change Pack`):
...
→ If the sprint is already sealed: make the change in the newest open sprint, or new sprint if none is open
→ To inspect differences first: diff [phase]
→ Other: describe what you'd like to do
```

When the user points at an older sprint, do not treat it as off-limits merely for being older — resolve the target via `core/sprint-manager.md § Cross-Sprint Routing`: an explicitly named or in-progress **unsealed** older sprint is a valid routed target, while a **sealed** sprint stays read-only and an in-place edit of an approved doc is still refused per the flow above. If the routed change touches an anchor a newer open sprint also carries, warn the user to re-review that newer sprint.
...
- **Never overwrite approved documents** — if the phase is already `APPROVED` in an open sprint, the import folds into a change pack (`core/import-validator.md § Import Into A Change Pack`); route everything else by `core/sprint-manager.md § Cross-Sprint Routing` (sealed → newest open sprint, or `new sprint` if none is open; an explicitly named unsealed sprint is a valid target — being *older* is not a refusal)
