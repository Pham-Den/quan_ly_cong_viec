...
5. **Artifact depth requirements — non-negotiable**:
   ...
   - **erd**: EVERY entity MUST have:
     ...
     - Package-level: the Relationship Summary diagram is a fenced `mermaid` block whose diagram type is `erDiagram` — NO ASCII art, plaintext entity-relationship diagrams, `text` / `ascii` fences, or prose-only substitutes

   - **sequence**: EVERY `SEQ-NNN` item MUST have:
     - A fenced `mermaid` code block whose diagram type is `sequenceDiagram`
     - NO ASCII art, plaintext ladder diagrams, `text` / `ascii` code fences, or prose-only substitutes for sequence diagrams
     - Happy path and error path coverage for each core flow
     - For qualifying flows, include `[TX BEGIN]` / `[TX COMMIT/ROLLBACK]` transaction boundaries for multi-step DB writes, `[TO: Xms]` for every external service call, `[RETRY: n, strategy]` for retry intent, `[IDEMPOTENT: key=...]` for idempotent endpoints, and `[ASYNC]` for fire-and-forget operations
     - Missing any required item on a qualifying flow → sequence is INCOMPLETE
...
> **Reflect sprint (`kind: reflect` — `core/sprint-manager.md § Reflect Sprint`):** the `ARCH-1` C4 (3-level Draw.io/XML) and `ARCH-2` DFD asset mandates are **exempt** — a reflected as-built architecture records observed components, contracts, and data model, not freshly-authored C4/DFD diagram assets; run `validate_proposal.py` **without** `--check-c4 --check-dfd` (`VP-13`/`VP-14` only fire when the overview itself claims a C4/DFD model). The Planning-Ready mandate that every Must-Have FR trace to component / API / sequence relaxes (reflected FRs describe running code, not specified Must-Haves). All other items STILL apply: `validate_proposal.py` structural checks (anchors, `VP-11` `ARCH-OVERVIEW-001`-sync, `VP-12`, `VP-15` sequence Mermaid blocks, split-target prefixes), `LTV-COV` / `LTV-SHRINK` / `LTV-PKGIDX` (`ARCH-OVERVIEW-001` + §2 Package Index + traceability map — enforced at seal regardless of `kind`), and the active `validate architecture` file (its audit applies these same reflect exemptions: it must not raise C4/DFD/Planning-Ready blockers for an as-built reflect bundle).
>
> **Intent / analysis the code cannot supply is declared-deferred, NOT a blocker** (the cross-phase rule — `core/sprint-manager.md § Reflect Sprint` rule 8 "WHY-gap deferral"):
> - An observed **NFR with no target** or a missing Architecturally Significant Scenario → a row in `nfr.md` **§9 Open NFR Risks** (the fixed home, mirroring PRD §10b; risk-managed-`TBD` per `core/phase-quality-standards.md`), NOT a "NFR vague / unmeasurable" blocker. The NFR template's scenario sub-sections (Performance / Resilience / Scalability) the code cannot quantify are satisfied by `N/A — reflected, target deferred to §9`, never left blank (a blank required section is a `DOC-3` blocker).
> - An **observed decision whose rationale / trade-offs are not in the code** → recorded as observed, with the missing rationale noted in the ADR block (`Rationale: reflected — not recoverable from code`) **and logged as an open question in architecture §12 Risks And Open Questions** (the arch-side resurface home, mirroring PRD §11). "ADR without rationale" is then a quality finding, not a hard gate. Existing code that **deviates from current company standards** (Layer 3, which would otherwise be a "deviation without ADR justification" blocker) → recorded as an observed known-gap in **architecture §12**, NOT a blocker — the code already runs; a governed fix is a later forward sprint.
> - A **STRIDE threat model** the as-built never produced → `N/A — not performed at reflect (known gap)`, NOT a blocker.
> - The structural / contract obligations the code DOES supply (api-specs field schemas, ERD DDL, sequence TX/timeout, event DLQ/retry, project-reference, integration classification) STILL hold in full. The `validate architecture` three-layer audit applies these same deferrals. **Forward (non-reflect) sprints keep all of these in full force and ASK the user** — see the reflect-only guard in rule 8.
...
- `python .prism/core/tools/validate_proposal.py --file <each architecture proposal> --check-c4 --check-dfd` returns any blocker, including missing / malformed anchors, duplicate IDs, wrong split-target prefixes, malformed frontmatter, unknown top-level merge sections, unmergeable H2 structure inside an anchored block, a `## New` `ARCH-COMP` without an accompanying `ARCH-OVERVIEW-001` New/Updated block (`VP-11`), a table in the proposal preamble (`VP-12`), a `SEQ-NNN` item that lacks a fenced Mermaid `sequenceDiagram` block or uses ASCII/plaintext ladder art instead (`VP-15`), or — when the `architecture-v{X}.md` overview claims a C4 model — a referenced C4 Draw.io/XML source that is missing, lacks one of the 3 required levels (System Context / Container / Component), or has an empty-stub level (`VP-13`, enabled by `--check-c4`; the 4th C4 "Code" level is not required), or — when `data-flow-v{X}.md` claims a DFD — a referenced DFD Draw.io/XML source that is missing or has no non-empty diagram page (`VP-14`, enabled by `--check-dfd`; missing / unlabeled arrows warn). Text↔diagram semantic agreement, DFD user-group splitting, notation shape-typing, and connector routing stay LLM-judged in the three-layer report
- `python .prism/core/tools/validate_living_truth.py --effective --sprint v{X} --candidate-phase architecture` returns any blocker — a Must Have FR (product effective truth) missing from the merged `ARCH-OVERVIEW-001` traceability map (`LTV-COV`), or an `## Updated ARCH-OVERVIEW-001` regeneration that silently dropped a still-live traceability row (`LTV-SHRINK`). These run the deterministic `ARCH-1` coverage + no-shrink checks on the pre-seal effective truth, so drift is caught at approve, not deferred to seal. `--candidate-phase architecture` is REQUIRED here: the gate runs while the architecture proposal is still `DRAFT` (its status flips to `APPROVED` only after the gate passes), so the candidate flag composes that DRAFT proposal as the approval candidate and answers "if I approve this draft, does coverage hold?". Without it the check would compose approved-only content, never see the trace the architect just authored in the DRAFT, and deadlock. The flag composes the DRAFT candidate but keeps every finding a hard blocker — a genuinely untraced Must FR still blocks approval. An empty / unfilled `ARCH-OVERVIEW-001` traceability map does NOT pass when Must FRs exist (an unfilled map is a coverage gap); the only FR-ref-free pass is an explicit `<!-- COVERAGE: N/A -->` marker in the overview block, reserved for a genuinely untraced scope (rare for architecture, which mandates the map). **When this same gate runs for a change-pack approval (`approve changes`)**, the architecture proposal is already `APPROVED` and the candidate is instead the pack's still-DRAFT delta(s): use `--candidate-phase architecture --candidate-pack v{X}.{Y}.{Z}-{slug}` (the pack folder name) — keep `--candidate-phase architecture` (it selects arch coverage; `--candidate-pack` alone is refused) and add `--candidate-pack`. The gate then composes `base + APPROVED deltas + this pack's DRAFT deltas` (per `core/change-manager.md § validate changes`) and answers "if I approve this pack, does coverage hold?". Without it a pack whose DRAFT arch delta carries the only trace for an already-approved Must FR could never be approved — the change-pack form of the same deadlock. Scoped to the named pack and to the sprint being approved; an unrelated DRAFT pack, or a pack from another sprint, is never pulled in.
...
- **Proposal structure check**: run `python .prism/core/tools/validate_proposal.py --file <each architecture proposal> --check-c4 --check-dfd`. Any blocker (malformed anchors, duplicate IDs, missing required frontmatter keys, missing routing tags, unknown top-level merge sections, unmergeable H2s inside anchored blocks, wrong split-target prefix, or `VP-15` sequence Mermaid violations) blocks `approve arch`; Updated/Removed target existence is confirmed by `seal_sprint.py` against routed LT files. The sequence check requires each `SEQ-NNN` New/Updated item to contain a fenced `mermaid` code block whose diagram type is `sequenceDiagram`; ASCII/plaintext ladder diagrams are blockers, not acceptable textual equivalents. The `--check-c4` flag adds `VP-13`: when the `architecture-v{X}.md` overview claims a C4 model (references an `assets/c4-*.drawio` source or carries a C4 section), that Draw.io/XML source must exist and cover the 3 required levels (System Context, Container, Component), each non-empty. The `--check-dfd` flag adds `VP-14`: when `data-flow-v{X}.md` claims a DFD (references an `assets/dfd-*.drawio` source or carries a DFD section), that source must exist and hold at least one non-empty diagram page (`ARCH-2` requires ≥ 1 DFD), with warnings for pages that have no arrows or unlabeled arrows. Together these deterministically back the C4 / DFD fail-fast rules above. The 4th C4 level (Code) is not required; semantic agreement between diagram shapes and the text tables, DFD notation shape-typing, user-group splitting, and connector routing stay manual Layer-1 cross-checks.
- `DOC-3`: required sections / fields from all Architecture templates are present or explicitly marked N/A with reason.
- `VAL-1`: the active validate file records structural coverage and rule coverage for `DOC-1`, `DOC-2`, `DOC-3`, `LINK-1`, `LINK-2`, `ORB-1`, `SEM-1`, `ARCH-1`, and `ARCH-2`.
- `SEM-1`: run the semantic integrity pass across architecture effective truth, approved Product, optional Design context, and all architecture companion proposals: catch text vs diagram/API/ERD/sequence/NFR/ADR contradictions, duplicated components / APIs / entities under different IDs, terminology or module-name drift from glossary / project-reference, stale assumptions from earlier sealed truth or change packs, and redundant or out-of-scope architecture decisions.
...
- every New/Updated `SEQ-NNN` item uses fenced Mermaid `sequenceDiagram`; ASCII/plaintext ladder diagrams are blockers
- the Relationship Summary diagram in `/docs/architecture/erd.md` uses a fenced Mermaid `erDiagram`; ASCII/plaintext entity-relationship art or prose-only substitutes are blockers (Layer-1, LLM-judged — no deterministic VP backstop)
