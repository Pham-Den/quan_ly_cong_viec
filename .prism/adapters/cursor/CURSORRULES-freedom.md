...
A future product idea or "note for later" that is not this-phase work → offer to save it to the Product Backlog (`docs/product-backlog.md`) per `core/phase-product.md § Product Backlog (PROD-6)`; write only on explicit / confident intent, else ask.
...
- `validate_implementation.py` is likewise NOT enforced as a gate. When auditing implemented code on request, still apply the `VAL-2` reviewer stance from `core/phase-implement.md § Independent verification protocol` (fresh-context reviewer agent by default — same conduct and reviewer input contract as guided, just with nothing blocking; re-derive expectations from the project docs, refute per AC, re-run evidence), and optionally run the tool as a sanity check; nothing blocks on its findings. When auditing a document on request, likewise apply the `VAL-3` stance (`core/phase-quality-standards.md § General Standards`): a fresh-context reviewer agent by default, re-read the target / template / upstream docs from files, re-derive expectations before trusting in-conversation claims, and search for each rule's fail signal — with nothing blocking on the findings.
...
# Deterministic implementation-evidence checks (declared test delta vs actual diff,
# CODE-1 marker facts, technique-evidence table, plan validation commands via --run)
python .prism/core/tools/validate_implementation.py --plan <plan> --task-group <id>
```
...
- Every new or materially changed in-scope meaningful backend/runtime, frontend/UI, or AI/ML code surface must satisfy the canonical `CODE-1 Traceability Contract` in `core/phase-implement.md §6` (loaded on implement intent). Include explicit pack / `Tracking IDs / ticket / task / board IDs` only when provided — never invent them. Preserve or update these markers on `feedback:`. Do not spray boilerplate markers across trivial helpers (`CODE-2`).
...
- Read existing codebase first. Follow existing patterns before creating new ones. `CODE-1` overrides local no-comment legacy style; missing old markers are not a pattern to copy.
